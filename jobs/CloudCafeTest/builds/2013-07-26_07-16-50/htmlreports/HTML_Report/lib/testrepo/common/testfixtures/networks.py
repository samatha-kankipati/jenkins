...
class DualModeFixture(BaseNetworksFixture):
    ...
    def setUpClass(cls):
        ...
        cls.isolated_network_A = cls.create_ipv4_network(
            name_starts_with='dual_netA')
        cls.isolated_network_B = cls.create_ipv4_network(
            name_starts_with='dual_netB')
        ...
        # Expected IP counts based on server data setUp
        cls.public_ips_count = cls.count_ips(cls.server_data, 'public')
        cls.private_ips_count = cls.count_ips(cls.server_data, 'private')
        cls.isolated_A_ips_count = cls.count_ips(cls.server_data, 'isolated_A')
        cls.isolated_B_ips_count = cls.count_ips(cls.server_data, 'isolated_B')
        ...
        # Add server and IP data to server_data list to use in tests
        for index, server in enumerate(cls.servers):
            public_ip = cls.helper.get_server_ips(server, cls.public_network)
            private_ip = cls.helper.get_server_ips(server, cls.private_network)
            isolated_A_ip = cls.helper.get_server_ips(server,
                cls.isolated_network_A)
            isolated_B_ip = cls.helper.get_server_ips(server,
                cls.isolated_network_B)
            cls.server_data[index]['server_id'] = server.id
            cls.server_data[index]['adminPass'] = server.adminPass
            cls.server_data[index]['all_ips'] = []
            if public_ip:
                cls.server_data[index]['public_ip'] = public_ip[0]
                cls.server_data[index]['all_ips'].append(public_ip[0])
            else:
                cls.server_data[index]['public_ip'] = None
            if private_ip:
                cls.server_data[index]['private_ip'] = private_ip[0]
                cls.server_data[index]['all_ips'].append(private_ip[0])
            else:
                cls.server_data[index]['private_ip'] = None
            if isolated_A_ip:
                cls.server_data[index]['isolated_A_ip'] = isolated_A_ip[0]
                cls.server_data[index]['all_ips'].append(isolated_A_ip[0])
            else:
                cls.server_data[index]['isolated_A_ip'] = None
            if isolated_B_ip:
                cls.server_data[index]['isolated_B_ip'] = isolated_B_ip[0]
                cls.server_data[index]['all_ips'].append(isolated_B_ip[0])
            else:
                cls.server_data[index]['isolated_B_ip'] = None
        ...
        for server in cls.server_data:
            if server['public_ip'] is not None:
                cls.public_ips.append(server['public_ip'])
        cls.assert_data(len(cls.public_ips), cls.public_ips_count,
            msg.format(cls.public_network.label, len(cls.public_ips),
                cls.public_ips_count))

        cls.private_ips = []
        for server in cls.server_data:
            if server['private_ip'] is not None:
                cls.private_ips.append(server['private_ip'])
        cls.assert_data(len(cls.private_ips), cls.private_ips_count,
            msg.format(cls.private_network.label, len(cls.private_ips),
                cls.private_ips_count))

        cls.isolated_A_ips = []
        for server in cls.server_data:
            if server['isolated_A_ip'] is not None:
                cls.isolated_A_ips.append(server['isolated_A_ip'])
        cls.assert_data(len(cls.isolated_A_ips), cls.isolated_A_ips_count,
            msg.format(cls.isolated_network_A.label, len(cls.isolated_A_ips),
                cls.isolated_A_ips_count))

        cls.isolated_B_ips = []
        for server in cls.server_data:
            if server['isolated_B_ip'] is not None:
                cls.isolated_B_ips.append(server['isolated_B_ip'])
        cls.assert_data(len(cls.isolated_B_ips), cls.isolated_B_ips_count,
            msg.format(cls.isolated_network_B.label, len(cls.isolated_B_ips),
                cls.isolated_B_ips_count))

    @classmethod
    def count_ips(cls, dict_list, key):
        """
        @summary: Counts keys with True values in all dictionaries
    ...
    def verify_remote_ping(cls, target_network, ssh_network='public',
        ...
        @type: int
        """
        ips_str = '{0}_ips'.format(target_network)
        remote_ips = getattr(cls, ips_str)

        for index, server in enumerate(cls.servers):
            if cls.server_data[index][ssh_network] and \
                cls.server_data[index][target_network]:
