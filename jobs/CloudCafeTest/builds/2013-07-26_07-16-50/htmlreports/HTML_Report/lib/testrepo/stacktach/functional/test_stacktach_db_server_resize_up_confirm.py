...
class StackTachDBResizeServerUpConfirmTests(STConfirmResizeServerFixture):
    '''
    ...
    @classmethod
    def setUpClass(cls):
        super(StackTachDBResizeServerUpConfirmTests, cls).setUpClass()

    @classmethod
    def tearDownClass(cls):
        super(StackTachDBResizeServerUpConfirmTests, cls).tearDownClass()
    ...
    def test_launch_entry_fields_on_resize_up(self):
        ...
        self.assertEqual(self.confirmed_resized_server.id, self.st_launch_resize_server.instance,
                        self.msg.format("instance",
                                        self.confirmed_resized_server.id,
                                        self.st_launch_resize_server.instance,
                                        self.st_launch_response.reason,
                                        self.st_launch_response.content))
        self.assertEqual(self.confirmed_resized_server.flavor.id,
                         self.st_launch_resize_server.instance_type_id,
                         self.msg.format("instance_type_id",
                                         self.confirmed_resized_server.flavor.id,
                                         self.st_launch_resize_server.instance_type_id,
                                         self.st_launch_response.reason,
                                         self.st_launch_response.content))
    ...
    def test_exist_entry_on_resize_up_server_response(self):
        ...
        self.assertTrue(self.st_exist.usage,
                        self.msg.format("usage",
                                        "Not None or Not Empty", self.st_exist.usage,
                                        self.st_exist_response.reason,
                                        self.st_exist_response.content))
        # We do not delete the server so this should be null.
