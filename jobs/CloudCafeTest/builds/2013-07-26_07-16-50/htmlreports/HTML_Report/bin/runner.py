#!/usr/bin/env python
'''The Default Runner for Cloudcafe'''
...
try:
    from ccengine.common.reporting.parse_result import ParseResult
    from testrepo.common.testfixtures.fixtures import BaseTestFixture
except ImportError:
    print 'SET YOUR PYTHON PATH! export PYTHONPATH=$PYTHONPATH:%s' % \
...
LOG_BASE_PATH = os.path.expanduser('{0}/.cloudcafe/logs'.format(BASE_DIR))
DATA_DIR = os.path.expanduser('{0}/.cloudcafe/data'.format(BASE_DIR))

#-----------------------Debug Logger-----------------------------
#from StringIO import StringIO

#from ccengine.common.tools import logging_tools
#logger = logging.getLogger('ccengine')
#handler = logging_tools.setup_new_cchandler('ccengine_log')
#logger.addHandler(handler)
#logger.debug('TEST RUNNER LOG!')
...
#        # be nice if TextTestRunner just had a class attr for
#        # defaultResultClass
#        return LogCaptureResult(self.stream, self.descriptions,
#                                                  self.verbosity)
#-----------------------Debug Logger-----------------------------
...
class CCRunner(object):
    ...
    def get_cl_args(self):
        ...
        *list tests and configs for a product
        ...
        specified).
        """ % (mug, help1, help2, help3, help4, export_path)
        ...
        argparser.add_argument('-p', '--packages',
                               ...
                               help="test package(s) in the product's "
                                    "test repo")
        argparser.add_argument('-P', '--nested-packages',
                               nargs='*',
                               default=None,
                               metavar='[package(s)]',
                               help="List of nested packages. Expects each "
                                    "package after the first to be a"
                                    "subpackage of the package before it in "
                                    "the list")
    ...
    def tree(self, directory, padding, print_files=False):
        ...
        if print_files:
            try:
                ...
                print self.colorize('Config Does Not Exist', HIGHLIGHTED_RED)
        else:
    ...
    def print_product_log_path(self, product_log_path):
        '''
        prints out the path to the log
        '''
        print '=' * 150
        print ("Log File Path: %s" % (product_log_path))
        print '=' * 150
    ...
    def set_env(self, config_path, log_path, data_dir):
        ...
        os.environ['CLOUDCAFE_TEST_CONFIG'] = config_path
    ...
    def get_repo_path(self, parent_path, product):
        '''
        ...
        if product is not None:
            repo_path = os.path.join(parent_path,
                                     'lib',
                                     'testrepo',
                                     product)
    ...
    def get_dotted_path(self, path, split_token):
        ...
        try:
            ...
            temp_path = path.split(split_token)[position]
            split_path = temp_path[1:].split(DIR_SEPR)
    ...
    def build_suite(self, loaded_module, method_regex, cl_tags, supress_flag):
        ...
        base_dotted_path = self.get_dotted_path(module_path, 'lib')
    ...
    def run(self):
        ...
        repo_path = self.get_repo_path(parent_path, cl_args.product)
        ...
        if cl_args.list is not None:
            for arg in cl_args.list:
                if arg == 'tests':
                    ...
                    banner = ''.join(['\n', '<[CONFIGS]>', '\n'])
                    path = os.path.join(parent_path, 'config', cl_args.product)
            ...
            if cl_args.parallel:
                test_runner = ccunittest.CCParallelTextTestRunner(verbosity=cl_args.quiet)
            else:
            ...
            self.print_product_log_path(product_log_path)

            #nested-packages
            if cl_args.nested_packages is not None:
                package_path = '/'.join(cl_args.nested_packages)
                package_path = "{0}/{1}".format(repo_path, package_path)
                print package_path
                for module in self.get_modules(package_path, module_regex):
                    try:
            ...
            if (cl_args.packages is None) and (cl_args.nested_packages is
                                               None):
                for module in self.get_modules(repo_path, module_regex):
                    ...
                    try:
                        ...
                        self.print_traceback()
                        continue
            elif (cl_args.packages is not None) and (cl_args.nested_packages is
                                                     None):
