...
class CinderShell_SmokeTest(BaseTestFixture):
    ...
    def test_create_volume(self):
        ...
        volume_size = int(self.config.cinder_shell.min_volume_size)

        volume = self.CinderShellProvider.create_volume(self.volume_name, volume_type_domain_object.Name, volume_size)
        self.assertIsNotNone(volume, "Unable to create new volume!")
        self.assertEqual(volume['Size'], str(volume_size), "Volume created with incorrect size")
    ...
    def test_create_volume_from_snapshot(self):
        ...
        volume_from_snapshot = self.CinderShellProvider.create_volume_from_snapshot(self.volume_from_snapshot_name, self.snapshot_name)
