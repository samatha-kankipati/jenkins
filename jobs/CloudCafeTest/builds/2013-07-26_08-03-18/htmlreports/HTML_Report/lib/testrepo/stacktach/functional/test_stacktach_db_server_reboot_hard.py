...
from testrepo.common.testfixtures.stacktach_db_compute_integration \
        import STRebootServerHardFixture


class StackTachDBRebootHardServerTests(STRebootServerHardFixture):
    '''
    ...
    def test_launch_entry_fields_on_reboot_hard_server_response(self):
        ...
        self.assertEqual(self.hard_rebooted_server.id, self.st_launch_create_server.instance,
                        self.msg.format("instance",
                                        self.hard_rebooted_server.id,
                                        self.st_launch_create_server.instance,
                                        self.st_launch_response.reason,
                                        self.st_launch_response.content))
        self.assertEqual(self.hard_rebooted_server.flavor.id,
                         self.st_launch_create_server.instance_type_id,
                         self.msg.format("instance_type_id",
                                         self.hard_rebooted_server.flavor.id,
                                         self.st_launch_create_server.instance_type_id,
                                         self.st_launch_response.reason,
                                         self.st_launch_response.content))
    ...
    def test_no_delete_entry_on_reboot_hard_server_response(self):
        '''
        Verify that there is no delete entry after peforming a hard reboot
        on an instance.
        '''
    ...
    def test_no_exist_entry_on_reboot_hard_server_response(self):
        '''
        Verify that there is no exist entry peforming a hard reboot
        on an instance.
        '''
