...
class BasePersistentLinuxClient(object):
    ...
    def get_file_md5hash(self, filepath):
        ...
        result = re.findall(r"([a-fA-F\d]{32})", sshresp)
        self.client_log.debug("RAW SSH HASH: {0}".format(result))
        file_hash = None
        try:
            file_hash = result[0]
        except:
            self.client_log.debug("md5 sum of remote file failed")

        self.client_log.debug("REGEX FILE HASH: {0}".format(file_hash))
