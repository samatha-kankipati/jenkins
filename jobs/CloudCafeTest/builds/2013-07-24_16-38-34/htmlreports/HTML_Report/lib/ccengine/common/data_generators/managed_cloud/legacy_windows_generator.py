...
class ManagedCloudLegacyWindowsDataGenerator(DataGenerator):
    def __init__(self):
        config = _MCP()
        ...
        self.test_records = []
        server_name = "Legacy-Managed-Windows-Server"
        json_resp = json.loads(legacy_images_client.list_images().text)
        server_images_list = json_resp['images']
        for x in server_images_list:
            if("windows" in (x['name']).lower()):
                legacy_serv_client.create_server(name=rand_name(
                    server_name), flavor_id=4, image_id=x['id'])
