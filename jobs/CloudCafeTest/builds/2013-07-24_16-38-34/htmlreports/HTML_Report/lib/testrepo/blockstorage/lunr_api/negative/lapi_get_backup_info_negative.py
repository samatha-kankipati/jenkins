'''
@summary: Negative Lunr API Get Backup Volume Info Tests
...
class LunrGetBackupInfoAPINegativeTest(LunrAPIFixture):
    '''
    @summary: Negative Lunr API Get Backup Volume Info Tests
    '''
    @classmethod
    def setUpClass(cls):
        super(LunrGetBackupInfoAPINegativeTest, cls).setUpClass()
    ...
    def tearDownClass(cls):
        ...
        super(LunrGetBackupInfoAPINegativeTest, cls).tearDownClass()

    def test_get_info_on_nonexistent_backup(self):
        nonexistent_backup_name = 'ThisBackupShouldNotExist'
        expected_status_code = HttpStatusCodes.NOT_FOUND
        api_response = self.common_user_client.Backups.get_info(nonexistent_backup_name)
        ...
        self.assertEqual(json.loads(api_response.content)['reason'], "Cannot show non-existent backup '%s'" % nonexistent_backup_name)

    def test_get_info_on_backup_from_another_account(self):
        expected_status_code = HttpStatusCodes.NOT_FOUND
        user_account_b = self.LunrAPIProvider.create_user_client()
        self.account_clients_to_cleanup.append(user_account_b)
        volume = self.LunrAPIProvider.create_volume_for_user(self.common_user_client,self.vtypes[0])
        backup = self.LunrAPIProvider.create_volume_backup(volume.id, self.common_user_client)

        api_response = user_account_b.Volumes.get_info(backup.id)
        self.assertEqual(api_response.status_code, expected_status_code, "Status code %s was returned, instead of the expected %s"
                                                                % (api_response.status_code, expected_status_code))
        self.assertEqual(json.loads(api_response.content)['reason'], "Cannot show non-existent volume '%s'" % backup.id)
