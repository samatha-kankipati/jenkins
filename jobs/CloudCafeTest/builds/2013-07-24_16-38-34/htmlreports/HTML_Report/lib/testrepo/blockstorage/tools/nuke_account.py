...
class VolumeAttachmentsAPISmoke(CloudBlockStorage_IntegrationFixture):
    '''
    ...
    #Get all servers
        ...
        for server in servers:
            ...
            try:
                for server_attachment in server_attachments:
                    ...
                    resp = self.volume_attachments_api_provider.\
        ...
        for server in servers:
            ...
            resp = self.compute_api_provider.servers_client.delete_server(server.id)
            print "(failed with {0}: )".format(resp.status_code, resp.msg) if not resp.ok else "(suceeded)"
            if not resp.ok:
                trouble_servers.append(server.id)

    #Delete all Snapshots
        resp = self.volumes_api_provider.volumes_client.list_all_snapshots()
        snapshots = resp.entity or []
        for snapshot in snapshots:
            print "Deleting snapshot {0}".format(snapshot.id)
            resp = self.volumes_api_provider.volumes_client.delete_snapshot(snapshot.id)
            print "(failed with {0})".format(resp.status_code) if not resp.ok else "(suceeded)"
            if not resp.ok:
                trouble_snapshots.append(snapshot.id)

    #Delete all Volumes
        resp = self.volumes_api_provider.volumes_client.list_all_volumes()
        volumes = resp.entity or []
        for volume in volumes:
            print "Deleting volume {0}".format(volume.id)
            resp = self.volumes_api_provider.volumes_client.delete_volume(volume.id)
            print "(failed with {0})".format(resp.status_code) if not resp.ok else "(suceeded)"
            if not resp.ok:
