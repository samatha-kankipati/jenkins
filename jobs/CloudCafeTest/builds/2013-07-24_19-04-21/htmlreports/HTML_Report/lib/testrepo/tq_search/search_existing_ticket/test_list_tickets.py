...
class TestListTickets(TQSearchFixture):
    ...
    def test_list_tickets_by_multiple_values_for_equal_operator(self):
        ...
        '''
        cust_condition = \
            WhereIn("status_name", [self.status_1, self.status_3]).AND(
                WhereGreaterOrEquals("created", self.start_time)).AND(
                    WhereLessOrEquals("created", self.end_time))
        ticket_list_from_core = \
            self.core_provider.ticket_client.list_tickets_by_ticket_attributes(
        ...
        results = \
            self.gate_provider.search_client.search_ticket_in_core(
                created_at_range=
                "[{0},{1}]".format(self.utc_start_time, self.utc_end_time),
                status="{0},{1}".format(self.status_3, self.status_1)).entity
        results.tickets.sort(key=lambda x: x.number, reverse=False)
        self.assertTrue((len(ticket_list_from_core) == len(results.tickets))
                        and (len(ticket_list_from_core) != 0),
                        "The length of two lists does not match. {0} != {1}".
        ...
        cust_condition = \
            WhereEquals("status_name", self.status_1).AND(
                WhereEquals("queue_name", self.queue_name)).AND(
                    WhereGreaterOrEquals("created", "2013-02-10 03:00:26")).AND(
                        WhereLessOrEquals("created", self.end_time))
        ticket_list_from_core = \
            self.core_provider.ticket_client.list_tickets_by_ticket_attributes(
                cust_condition, attributes=self.core_attributes).entity
        ticket_list_from_core.sort(key=lambda x: x.number, reverse=False)
        not_equals_params = {'queue.name': self.queue_name}
        results = \
            self.gate_provider.search_client.search_ticket_in_core(
                hardcoded_parameters=not_equals_params,
                created_at_range=
                "[{0},{1}]".format("2013-02-10T08:00:26.000Z", self.utc_end_time),
                status="new").entity
        results.tickets.sort(key=lambda x: x.number, reverse=False)
    ...
    def test_list_tickets_query_params_with_single_value_and_not_eql_op(self):
        ...
        '''
        cust_condition = \
            WhereNotEquals("status_name", self.status_2).AND(
                WhereNotEquals("queue_name", self.queue_name_1)).AND(
                    WhereGreaterOrEquals("created", self.start_time)).AND(
        ...
        not_equals_params = {'status!': self.status_2,
                             'queue.name!': self.queue_name_1}
    ...
    def test_list_tickets_query_params_with_multiple_vals_and_not_eql_op(self):
        ...
        '''
        cust_condition = \
            WhereNotIn("status_name", [self.status_1, self.status_4]).AND(
                WhereGreaterOrEquals("created", self.start_time)).AND(
                    WhereLessOrEquals("created", self.end_time)).AND(
                        WhereEquals("status_type", self.status_type))
        ticket_list_from_core = \
        ...
        not_equals_params = {'status!': '{0},{1},{2},{3}'.format(self.status_1,
                                                                 self.status_4,
                                                                 "Closed",
                                                                 "Solved")}
    ...
    def test_list_tickets_eql_and_not_eql_op_single_val_in_query_params(self):
        ...
        '''
        cust_condition = \
            WhereNotEquals("priority", self.priority_3).AND(
                WhereEquals("queue_name", self.queue_name)).AND(
                    WhereGreaterOrEquals("created","2013-02-10 03:00:26")).AND(
                        WhereLessOrEquals("created", self.end_time)).AND(
                            WhereEquals("status_type", self.status_type))
        ticket_list_from_core = \
            self.core_provider.ticket_client.list_tickets_by_ticket_attributes(
                cust_condition, attributes=self.core_attributes).entity
        ticket_list_from_core.sort(key=lambda x: x.number, reverse=False)
        not_equals_params = {'priority!': self.tq_search_priority_2,
                             'queue.name': self.queue_name}
        results = \
            self.gate_provider.search_client.search_ticket_in_core(
                hardcoded_parameters=not_equals_params,
                created_at_range=
                '[{0},{1}]'.format("2013-02-10T08:00:26.000Z",
                                   self.utc_end_time)).entity
        results.tickets.sort(key=lambda x: x.number, reverse=False)
        self.assertTrue((len(ticket_list_from_core) == len(results.tickets))
                        and (len(ticket_list_from_core) != 0),
    ...
    def test_list_tickets_eql_and_noteql_op_multiple_vals_in_query_parms(self):
        ...
        cust_condition = \
            ...
            AND(WhereNotIn("status_name",
                           [self.status_2, self.status_3])).\
            AND(WhereGreaterOrEquals("created", self.start_time)).\
            AND(WhereLessOrEquals("created", self.end_time)).\
        ...
        ticket_list_from_core.sort(key=lambda x: x.number, reverse=False)
        not_equals_params = \
            {'status!': '{0},{1}'.format(self.status_2, self.status_3)}
    ...
    def test_list_tickets_by_date_range_in_query_params_with_eql_op(self):
        ...
        '''
        cust_condition = \
            WhereGreaterOrEquals("created", "2013-03-12 05:00:26").AND(
                WhereLessOrEquals("created", self.end_time)).AND(
                    WhereEquals("status_type", self.status_type))
        ticket_list_from_core = \
            self.core_provider.ticket_client.list_tickets_by_ticket_attributes(
        ...
        results = \
            self.gate_provider.search_client.search_ticket_in_core(
                created_at_range=
                '[{0},{1}]'.format("2013-03-12T10:00:26.000Z",
                                   self.utc_end_time)).entity
        self.assertTrue((len(ticket_list_from_core) == len(results.tickets))
                        and (len(ticket_list_from_core) != 0),
                        "The length of two lists does not match. {0} != {1}".
    ...
    def test_list_tickets_by_date_range_in_query_params_with_not_eql_op(self):
        ...
        for "!=" operator
        '''
        cust_condition = WhereLess("created", self.start_time_2).AND(
            WhereEquals("status_type", self.status_type))
        ticket_list_from_core = \
            self.core_provider.ticket_client.list_tickets_by_ticket_attributes(
                cust_condition, attributes=self.core_attributes).entity
        ...
        not_equals_params =\
            {'createdAt_range!': '[{0},{1}'.format(self.utc_start_time_2,
                                                   current_date_in_utc) + ']'}
