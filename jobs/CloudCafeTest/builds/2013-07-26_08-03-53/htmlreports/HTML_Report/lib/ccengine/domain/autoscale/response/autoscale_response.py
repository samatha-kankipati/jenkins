...
from ccengine.domain.autoscale.response.group_response import Active, Pending,\
    Server, Lbaas
from ccengine.domain.autoscale.servers import Metadata, Links
import json
import re
...
class ScalingGroup(BaseMarshallingDomain):
    '''
    works for the autoscaling groups manifest
    '''

    def __init__(self, **kwargs):
        super(ScalingGroup, self).__init__(**kwargs)
...
class Group(BaseMarshallingDomain):
    ...
    def _dict_to_obj(cls, group_dict):
        ...
        if hasattr(group, 'steadyState'):
            setattr(group, 'steadyState', getattr(group, 'steadyState'))
        if hasattr(group, 'paused'):
            setattr(group, 'paused', getattr(group, 'paused'))
...
class Config(BaseMarshallingDomain):
    '''
    works for the autoscaling groups configs amd launch configs
    '''
...
class Policy(BaseMarshallingDomain):
    '''
    ...
    def _dict_to_obj(cls, policy_dict):
        ...
        attr_list = ['id', 'name', 'change', 'changePercent', 'steadyState', 'cooldown']
        for k in attr_list:
            if hasattr(policy, k):
...
class Webhook(BaseMarshallingDomain):
    '''
