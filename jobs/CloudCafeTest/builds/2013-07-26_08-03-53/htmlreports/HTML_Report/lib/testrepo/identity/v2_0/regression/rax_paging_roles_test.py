...
class GetRolesPagingTest(BaseIdentityFixture):
    ...
    def setUpClass(cls):
        ...
        cls.service_client.token = cls.provider.get_token(
            username=cls.config.identity_api.service_username,
            password=cls.config.identity_api.service_password)
    ...
    def test_list_roles_returns_valid_first_link_when_on_last_page(self):
        ...
        response = self._follow_link('first',
                                     ...
                                     self.service_client.list_roles)
        self._status_assertion(response.status_code, 200)
        self.assertNotEqual(len(response.entity), 0,
                            msg='first link search is empty')
    ...
    def test_list_roles_returns_no_links_when_indexed_past_total_results(self):
        ...
        """
        response = self.service_client.list_roles(limit=1,
                                                  marker=1000000000000)
