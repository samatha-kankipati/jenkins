...
class VolumesAPI_Usage_Smoke(AtomhopperIntegrationFixture):
    ...
    def load_usage_data(self):
        ...
        for f in os.listdir('./'):
            if re.search('volumepickle*', f):
                ...
                m = re.search(
                    r'([0-9][0-9][0-9][0-9])-([0-1][0-9])-([0-9][0-9])',
                    c_date)
                ...
                data.append(
                    (search_attrib, search_regex, cutoff_attrib, cutoff_regex))
        return data
    ...
    def test_verify_usage(self):
        ...
        usage_data = [
            ('resource_id', 'TestVolume_854868', 'start_time', '2012-09-18T')]
        for data in usage_data:
            ...
            results[search_regex] = self.atomhopper_provider.\
                search_past_events_by_attribute(
                    search_attrib, search_regex, cutoff_attrib, cutoff_regex)
