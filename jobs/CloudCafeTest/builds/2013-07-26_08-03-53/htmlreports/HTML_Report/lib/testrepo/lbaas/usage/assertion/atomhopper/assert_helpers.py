...
def verify_record_properties(record, numVips=1, bandWidthIn=0,
    ...
    assert float(avgConcurrentConnections) - ACC_ACCEPTANCE_RANGE <= float(
        ...
        ('Expected: {0}, Actual: {1}'.format(
            ...
            float(record.avgConcurrentConnections) * int(record.numPolls)))

    assert float(avgConcurrentConnectionsSsl) - ACC_ACCEPTANCE_RANGE <= float(
