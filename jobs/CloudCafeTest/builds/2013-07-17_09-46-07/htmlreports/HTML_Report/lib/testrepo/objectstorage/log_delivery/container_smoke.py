...
class LogDeliveryContainerSmokeTest(ObjectStorageTestFixture):
    @classmethod
    def setUpClass(cls):
        super(LogDeliveryContainerSmokeTest, cls).setUpClass()

        cls.container_name = cls.client.generate_unique_container_name(
                '_log_delivery_container_smoke_tests')
        metadata = {'Access-Log-Delivery': 'True'}
        cls.client.create_container(cls.container_name,
                                                 metadata=metadata)
        cls.fixture_log.info(
                'Container created for container smoke log delivery tests(%s)'
                % cls.container_name)
    ...
    def test_container_creation_with_valid_container_name(self):
        # Since the we are creating the container in this test, we won't use
        #   the common container.  Instead we will create a new container using
        #   the common test container's name appended with '_create_test'

        container_name = ''.join([self.container_name, '_create_test'])
        metadata = {'Access-Log-Delivery': 'True'}
