...
class CreateScalingGroupTest(AutoscaleFixture):
    '''
    Assumes deleting the scaling group deletes the associated scaling policies
    '''
    @classmethod
    def setUpClass(cls):
        ...
        cls.lc_networks = [{'uuid': '11111111-1111-1111-1111-111111111111'}, {'uuid': '00000000-0000-0000-0000-000000000000'}]
        cls.lc_load_balancers = [{"loadBalancerId": 9099, "port": 8080}]
        cls.sp_list = [{"name": "scale up by 10", "change": 10, "cooldown": 5}]
    ...
    def test_create_scaling_group_response(self):
        ...
        @todo: Include validation of the headers and try to make this common
               for all calls by just varying the header info and the
               response code
        '''
        self.assertTrue(self.create_resp.ok,
                        msg="Create scaling group call failed with API Response: %s"
                        % self.create_resp.content)
        self.assertEquals(self.create_resp.status_code, 201,
                          msg="The create failed with %s"
                          % self.create_resp.status_code)
    ...
    def test_created_scaling_group_groupconfig_fields(self):
        '''Verify that a created scaling group has all the expected fields
           for group configuration'''
        '''assert name set during creation is same as name of server'''
        self.assertEqual(self.gc_name, self.scaling_group.groupConfiguration.name,
        ...
        self.assertEqual(self.gc_metadata,
                         self.autoscale_provider.to_data(self.scaling_group.groupConfiguration.metadata),
    ...
    def test_created_scaling_group_launchconfig_scalingpolicy_fields(self):
        ...
        '''assert name set during creation is same as name of server'''
        self.assertEqual(self.config.autoscale.lc_name,
                         self.scaling_group.launchConfiguration.server.name,
                         msg="Server name provided in the launch config did not match")
        self.assertEqual(self.config.autoscale.lc_image_ref,
                         self.scaling_group.launchConfiguration.server.imageRef,
                         msg="Image id did not match")
        self.assertEqual(self.config.autoscale.lc_flavor_ref,
                         self.scaling_group.launchConfiguration.server.flavorRef,
                         msg="Flavor id did not match")
        self.assertEquals(self.autoscale_provider.network_uuid_list(self.lc_networks),
                          self.autoscale_provider.network_uuid_list(self.scaling_group.launchConfiguration.server.networks),
                         msg="Networks within the launch config did not match")
        self.assertEquals(self.autoscale_provider.personality_list(self.lc_personality),
                          self.autoscale_provider.personality_list(self.scaling_group.launchConfiguration.server.personality),
                         msg="Personality within the launch config did not match")
        self.assertEqual(self.autoscale_provider.lbaas_list(self.lc_load_balancers),
                         self.autoscale_provider.lbaas_list(self.scaling_group.launchConfiguration.loadBalancers),
                         msg="Load balancers did not match")
        self.assertEqual(self.autoscale_provider.policy_details_list(self.sp_list),
                         self.autoscale_provider.policy_details_list(self.scaling_group.scalingPolicies),
