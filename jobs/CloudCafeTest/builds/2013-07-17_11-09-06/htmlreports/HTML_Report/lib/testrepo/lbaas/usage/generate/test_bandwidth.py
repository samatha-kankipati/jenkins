...
class TestBandwidth(LoadBalancersGenerateUsageFixture):
    ...
    def test_normal_bandwidth(self):
        ...
        bandwidth_out = self.lbaas_provider.generate_bandwidth_out(vip.address)
        bandwidth_in = self.lbaas_provider.generate_bandwidth_in(vip.address)
...
#        ssl_off_time = datetime.datetime.utcnow().isoformat()
    ...
    def test_ssl_bandwidth_across_ssl_states(self):
        ...
        vip = lb.get_public_ipv4_vip()
        bandwidth_out = self.lbaas_provider.generate_ssl_bandwidth_out(vip.address)
        bandwidth_in = self.lbaas_provider.generate_ssl_bandwidth_in(vip.address)
        ...
        bandwidth_out2 = self.lbaas_provider.generate_ssl_bandwidth_out(vip.address)
        bandwidth_in2 = self.lbaas_provider.generate_ssl_bandwidth_in(vip.address)
    ...
    def _generate_bandwidth_before_suspend_and_after_unsuspend(self,
        ...
        bandwidth_out2 = self.lbaas_provider.generate_bandwidth_out(
            vip.address)
        bandwidth_in2 = self.lbaas_provider.generate_bandwidth_in(vip.address)
    ...
    def test_normal_bandwidth_across_ssl_states(self):
        ...
        bandwidth_out2 = self.lbaas_provider.generate_bandwidth_out(
            vip.address)
        bandwidth_in2 = self.lbaas_provider.generate_bandwidth_in(
            vip.address)

        self.lbaas_provider.client.delete_ssl_termination(lb.id)
        r = self.lbaas_provider.wait_for_status(lb.id)
        self.assertEquals(LBST.ACTIVE, r.entity.status)
        bandwidth_out3 = self.lbaas_provider.generate_bandwidth_out(
            vip.address)
        bandwidth_in3 = self.lbaas_provider.generate_bandwidth_in(vip.address)
