...
class CinderAPISnapshotRegressionTests(VolumesAPI_Fixture):
    """
    ...
    def double_snapshot_create_conflict_regression(self, vol_type):
        ...
        resp = self.volumes_provider.create_available_volume(
            self.vol_name,
            self.volumes_provider.min_volume_size, vol_type)
        ...
        #Create a snapshot
        resp = self.volumes_client.create_snapshot(
        ...
        self.addCleanup(
            self.volumes_provider.delete_snapshot_confirmed,
            snapshot_1.id)
        ...
        resp = self.volumes_provider.wait_for_snapshot_status(
    ...
    def snapshot_stuck_in_deleting_regression(self, vol_type):
        ...
        resp = self.volumes_provider.create_available_volume(
            self.vol_name,
            self.volumes_provider.min_volume_size,
            vol_type)
        ...
        self.volume = resp.entity
        self.addCleanup(
            self.volumes_provider.delete_volume_confirmed, self.volume.id)

        #Create a snapshot of the volume
        resp = self.volumes_client.create_snapshot(
            self.volume.id, "{0}_snap1".format(self.vol_name))
        self.assertTrue(
            resp.ok, "Snapshot_1 create returned {0}, expected 200".format(
                resp.status_code))
        ...
        snapshot_1 = resp.entity
        self.addCleanup(
            self.volumes_provider.delete_snapshot_confirmed, snapshot_1.id)
        ...
        resp = self.volumes_client.create_snapshot(
            self.volume.id, "{0}_snap2".format(self.vol_name))
        self.assertTrue(
            resp.ok, "Second snapshot returned {0}, expected 200".format(
                resp.status_code))
        snapshot_2 = resp.entity
        self.addCleanup(
            self.volumes_provider.delete_snapshot_confirmed, snapshot_2.id)

        #While the second snapshot it creating, immediately attempt to
        #delete the first snapshot.  Expect success (This could change)
        resp = self.volumes_client.delete_snapshot(
            snapshot_1.id)
        self.assertTrue(
            resp.ok,
