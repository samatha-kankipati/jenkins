...
class TestQonosListJobs(BaseImagesFixture):
    ...
    def test_happy_path_list_jobs(self):
        ...
        self.assertTrue(len(listed_jobs) == count,
        ...
        self.assertTrue(self.images_provider.\
                        is_job_status_valid(listed_job.status),
                        msg.format('status', job.status, listed_job.status))
        ...
        self.assertTrue(listed_job.retry_count == job.retry_count or
                        listed_job.retry_count == 1,
        ...
        self.assertTrue(listed_job.worker_id == job.worker_id or
                        id_re.match(listed_job.worker_id) != None,
    ...
    def test_list_jobs_with_multiple_jobs(self):
        ...
        for listed_job in listed_jobs:
            ...
            self.assertTrue(self.images_provider. \
                            is_job_status_valid(current_job.status),
                            msg.format('status', current_job.status,
                                       listed_job.status))
            self.assertTrue(hard_timeout_diff_in_sec <= 10,
            ...
            self.assertTrue(listed_job.retry_count == current_job.retry_count
                            or listed_job.retry_count == 1,
                            msg.format('retry_count', '0/1',
                                       listed_job.retry_count))
            self.assertEquals(listed_job.schedule_id, current_job.schedule_id,
                              msg.format('schedule_id',
                                         ...
                                         listed_job.schedule_id))
            self.assertTrue(listed_job.worker_id == current_job.worker_id
                            or id_re.match(listed_job.worker_id) != None,
                            msg.format('worker', 'None/Valid worker id',
                                       listed_job.worker_id))
            self.assertTrue(timeout_diff_in_sec <= 10,
    ...
    def test_list_jobs_after_a_job_completes(self):
        ...
        self.assertTrue(listed_jobs[0].status == ScheduledImagesJobStatus.DONE,
                        msg.format("job status", ScheduledImagesJobStatus.DONE,
