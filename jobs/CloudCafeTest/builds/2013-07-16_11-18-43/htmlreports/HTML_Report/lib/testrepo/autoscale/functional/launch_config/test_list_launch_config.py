...
class ListLaunchConfigTest(AutoscaleFixture):
    ...
    def setUpClass(cls):
        ...
        create_resp = cls.autoscale_provider.create_scaling_group_given(
                                                gc_name=cls.gc_name,
        ...
        cls.launch_config_response = cls.autoscale_client.view_launch_config(cls.group.id)
    ...
    def test_list_launch_config_response(self):
        ...
        self.assertEquals(self.launch_config.server.name, self.config.autoscale.lc_name,
                          msg="Prefix/Suffix server name in the launch config did not match")
        self.assertEquals(self.launch_config.server.flavorRef, self.config.autoscale.lc_flavor_ref,
                          msg="Server flavor in the launch config did not match")
        self.assertEquals(self.launch_config.server.imageRef, self.config.autoscale.lc_image_ref,
                          msg="Server ImageRef in the launch config did not match")
        self.assertEquals(self.autoscale_provider.personality_list(self.launch_config.server.personality),
            self.autoscale_provider.personality_list(self.lc_personality),
                          msg="Server personality in the launch config did not match")
        self.assertEquals(self.autoscale_provider.to_data(self.launch_config.server.metadata),
            self.lc_metadata,
                          msg="Server metadata in the launch config did not match")
        self.assertEquals(self.autoscale_provider.network_uuid_list(self.launch_config.server.networks),
            self.autoscale_provider.network_uuid_list(self.lc_networks),
                          msg="Server networks did not match")
        self.assertEquals(self.autoscale_provider.lbaas_list(self.launch_config.loadBalancers),
