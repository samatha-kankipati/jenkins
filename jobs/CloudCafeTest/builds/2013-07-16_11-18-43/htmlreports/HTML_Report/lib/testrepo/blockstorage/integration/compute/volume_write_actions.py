...
class VolumeWriteActions(BlockStorageLinuxIntegrationFixture):
    ...
    def test_restore_and_verify_backup_of_written_volume(self):
        ...
        assert resp.ok, "Unable to create a volume from the original snapshot"
        ...
        self.verify_remote_file_size(
            self.test_env.ssh_conn, file_block_size * file_block_count,
            "{0}/{1}".format(new_mount_point, file_name))
