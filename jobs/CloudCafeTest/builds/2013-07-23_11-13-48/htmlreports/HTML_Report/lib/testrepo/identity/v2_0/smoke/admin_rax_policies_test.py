...
class AdminPoliciesTest(IdentityAdminFixture):
    ...
    def test_get_all_policies(self):
        ...
        self.assertEqual(get_policies.status_code, 200,
                msg="Admin get all policies expected response 200 received %s"
                % get_policies.status_code)
    ...
    def test_create_policy(self):
        ...
        self.assertEqual(create_policy.status_code, 201,
                msg="Admin create policy expected response 201 received %s" %
                create_policy.status_code)

        policy_id = create_policy.headers['location'].split('/')[-1]

        self.addCleanup(self.admin_client.delete_policy, policyId=policy_id)
    ...
    def test_get_a_policy(self):
        ...
        self.assertEqual(get_policies.status_code, 200,
                msg="Admin get policies expected response 200 received %s" %
                get_policies.status_code)

        get_policy = self.admin_client.get_policy(
                policyId=get_policies.entity[0].id)

        self.assertEqual(get_policy.status_code, 200,
                msg="Admin get policy expected response 200 received %s" %
                get_policy.status_code)
    ...
    def test_update_policy(self):
        ...
        self.assertEqual(update_policy.status_code, 204,
                msg="Admin update policy expected response 204 received %s" %
                update_policy.status_code)
    ...
    def test_delete_policy(self):
        ...
        self.assertEqual(delete_policy.status_code, 204,
                msg="Admin delete policy expected response 204 received %s" %
    ...
    def test_get_detailed_policy_content(self):
        ...
        policy_detail = self.admin_client.get_detailed_policy_content(
                policyId=get_policies.entity[-1].id)

        self.assertEqual(policy_detail.status_code, 200,
                msg="Admin get detailed policy content expected response 200 "
                "received %s" % policy_detail.status_code)
