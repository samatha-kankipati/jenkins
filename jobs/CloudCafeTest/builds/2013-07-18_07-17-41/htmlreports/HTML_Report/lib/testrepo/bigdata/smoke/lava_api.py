...
class LavaAPITest(LavaBaseFixture):
    ...
    def test_update_profile(self):
        ...
        email = self.config.lava_api.PROFILE_EMAIL
        api_response = profiles_client.edit(email=email,
                                            username=
                                            self.config.lava_api.USER_NAME,
                                            password=
        ...
        self.assertEqual(profile_created.cloud_credentials.username,
                         ccs.username)
        self.assertEqual(profile_created.email,
                         email)
    ...
    def test_resize_cluster(self):
        ...
        self.assertTrue(response.ok, "API call to resize"
                        ...
                        % (self.cluster,
                           cluster_new_size,
                           response.content))
        wait_result, msg = self.lava_provider.wait_for_cluster_status(
            self.cluster.id, LavaClusterStatusTypes.ACTIVE)
