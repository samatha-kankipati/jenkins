...
from ccengine.domain.autoscale.servers import Metadata, Network, \
    Links, Personality
import re


class Active(BaseMarshallingDomain):
...
class Pending(BaseMarshallingDomain):
...
class Lbaas(BaseMarshallingDomain):

    def __init__(self, **kwargs):
        super(Lbaas, self).__init__(**kwargs)
        for keys, values in kwargs.items():
            setattr(self, keys, values)
