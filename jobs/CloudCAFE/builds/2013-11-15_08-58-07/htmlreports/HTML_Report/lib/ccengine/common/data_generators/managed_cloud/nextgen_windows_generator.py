...
class ManagedCloudWindowsNextGenDataGenerator(DataGenerator):

    def __init__(self):
        config = _MCP()
        servers_created = []
        self.test_records = []
        server_name = "{0}-Managed-Windows-Server".format(
        ...
        for image in windows_images_list:
            servers_created.append(
                compute_provider.create_server_no_wait(
                    ...
                    imageRef=image.id,
                    flavorRef=config.managedcloud.windows_flavor).entity)
