...
class Identity_SUF_IntegrationTestFixture(BaseTestFixture):
    ...
    def get_role(cls, user_client, user_id, role_name):
        role_list_limit = 1000
        resp = user_client.list_roles(limit=role_list_limit)
        assert resp.ok, 'Unable to list roles in test setup'
        roles = resp.entity
        assert roles is not None, 'Unable to deserialize role list'
        found_role = None
        for role in roles:
            if role.name == role_name:
                found_role = role

        assert found_role is not None, "The '{0}' role was not found.".format(
            role_name)
...
# admin_api
    ...
    def test_volume_detach(self):
        ...
        volume = self.setup_a_test_volume()
        vol_attach_timeout = 120

        #Make sure volume attach call succeeds
        resp = self.main_test_user_vol_attach_provider.client.attach_volume(
            server.id, volume.id)

        assert resp.entity is not None, (
            "Test setup failure: Call to os-volume_attachments failed to "
            "return a deserializable response for server {0} and volume {1}"
            .format(server.id, volume.id))

        assert resp.entity.id is not None, (
            "Test setup failure: Unable to retrieve volume attachment id from"
            "deserialized volume attach call for server {0} and volume {1}"
            .format(server.id, volume.id))

        #Make sure volume attachment propagates through the nova cells
        attachment = resp.entity
        is_attachment_available = self.main_test_user_vol_attach_provider\
            .wait_for_volume_attachment_to_propagate(
                attachment.id, server.id, timeout=vol_attach_timeout,
                wait_period=5)

        assert is_attachment_available is True, (
            "Test setup failure: Attachment {0} on server {1} for volume {2} "
            "did not propagate within the alloted timeout of {3} seconds."
            .format(attachment.id, server.id, volume.id, vol_attach_timeout))

        #Make sure the volume is in-use (attached)
        resp = self.main_test_user_volumes_provider.wait_for_volume_status(
            volume.id, 'in-use')
        assert resp.ok, (
            "Test setup failure: Volume {0} did not enter the 'in-use' status"
            "  within the alloted timeout")

        #Actual Test!! Make sure sub user can detach an attached, in-use volume
        ...
        try:
            ...
            self.fixture_log.exception(exception)
    ...
    def test_volume_attach(self):
        ...
        try:
            self.addCleanup(
                self.main_test_user_vol_attach_provider
                .detach_volume_confirmed, attachment.id, server.id)
        except Exception as exception:
            self.fixture_log.warning(
                "Unable to use addCleanup to delete volume attachment")
            self.fixture_log.exception(exception)
...
class RBAC_CBS_SubUserTestFixture(CBS_RBAC_IntegrationTestFixture):
    ...
    def tearDownClass(cls):
        print 'Running teardown by deleting test user: {0}'.format(
            cls.actual_test_user.id)
        cls.main_test_user.client.delete_user(cls.actual_test_user.id)
