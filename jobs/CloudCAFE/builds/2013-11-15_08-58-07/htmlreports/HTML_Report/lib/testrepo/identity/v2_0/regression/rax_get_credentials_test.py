...
class RaxGetCredentialsTest(BaseIdentityFixture):
    ...
    def tearDownClass(cls):
        cls.delete_user_permanently(user_id=cls.sub_user_id,
                                    client=cls.public_client)
        cls.delete_user_permanently(user_id=cls.user_adm_id,
                                    client=cls.admin_client)
        cls.delete_user_permanently(user_id=cls.iden_adm_id,
        ...
        for client in client_keys:
            ...
            for user_id in user_id_list:
                get_pass_credentials = client.get_user_credentials_password(
                    user_id=user_id)
                get_user = self.service_client.get_user_by_id(user_id=user_id)
