...
from testrepo.common.testfixtures.stacktach_db_compute_integration \
        import STRebootServerSoftFixture


class StackTachDBRebootSoftServerTests(STRebootServerSoftFixture):
    '''
    ...
    @classmethod
    def setUpClass(cls):
        super(StackTachDBRebootSoftServerTests, cls).setUpClass()

    @classmethod
    def tearDownClass(cls):
        super(StackTachDBRebootSoftServerTests, cls).tearDownClass()

    @attr(type='positive')
    def test_launch_entry_on_reboot_soft_server_response(self):
        '''
        Verify that the Launch parameters are being returned from the 
        StackTach DB after peforming a soft reboot on an instance.
        '''
    ...
    def test_launch_entry_fields_on_reboot_soft_server_response(self):
        ...
        self.assertEqual(self.soft_rebooted_server.id, self.st_launch_create_server.instance,
                        self.msg.format("instance",
                                        self.soft_rebooted_server.id,
                                        self.st_launch_create_server.instance,
                                        self.st_launch_response.reason,
                                        self.st_launch_response.content))
        self.assertEqual(self.soft_rebooted_server.flavor.id,
                         self.st_launch_create_server.instance_type_id,
                         self.msg.format("instance_type_id",
                                         self.soft_rebooted_server.flavor.id,
                                         self.st_launch_create_server.instance_type_id,
                                         self.st_launch_response.reason,
                                         self.st_launch_response.content))
    ...
    def test_no_delete_entry_on_reboot_soft_server_response(self):
        '''
        Verify that there is no delete entry after peforming a soft reboot
        on an instance.
        '''
    ...
    def test_no_exist_entry_on_reboot_soft_server_response(self):
        '''
        Verify that there is no exist entry peforming a soft reboot
        on an instance.
        '''
