...
class TestQonosDeleteScheduleNegative(BaseImagesFixture):
    ...
    @attr('negative')
    def test_delete_schedule_with_incorrect_url(self):
        """Delete schedule with incorrect url.

        1) Attempt to request the base url of '/scheduless' using a DELETE
            method
        ...
        sch_obj = self.images_provider.\
            create_active_schedules(tenant, action)
        self.assertEquals(sch_obj.status_code, 200,
                          msg.format('status_code', 200, sch_obj.status_code))

        sch = sch_obj.entity

        bad_url = "{0}/scheduless".format(self.config.images.url)

        with self.assertRaises(ItemNotFound):
            self.images_provider.schedules_client. \
                delete_schedule(sch.id, requestslib_kwargs={'url': bad_url})

    @attr('negative')
    def test_delete_schedule_without_schedule_id(self):
        """Delete schedule without scheduled id.
