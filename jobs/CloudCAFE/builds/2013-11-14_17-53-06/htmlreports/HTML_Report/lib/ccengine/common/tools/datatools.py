...
def string_to_datetime(datetimestring, date_formats=None):
    date_formats = date_formats or [
        ...
        '%Y-%m-%dT%H:%M:%S.000+0000', '%Y-%m-%d %H:%M:%S.000+0000',
...
def convert_camelcase_to_python_std_varible(value):
    """
    Converts the camel case variable to underscore separated  lower case
    variable(e.g TypeName -> type_name)
    """
    regx_one = '([a-z0-9])([A-Z])'
    regx_two = r'\1_\2'
    new_data = re.sub(regx_one, regx_two, value).lower()
    return new_data
