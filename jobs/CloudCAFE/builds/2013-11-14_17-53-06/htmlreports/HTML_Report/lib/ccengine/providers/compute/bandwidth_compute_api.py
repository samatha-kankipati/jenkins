...
from ccengine.common.tools.datatools import gb_to_bytes, string_to_datetime
...
class BandwidthComputeAPIProvider(ComputeAPIProvider):
    ...
    def __init__(self, config, logger=None):
        ...
        self.ssh_timeout = self.config.compute_api.ssh_timeout
    ...
    def rescue_and_unrescue(self, server_id):
        ...
        @return:  Unrescued server entity object
        @note: Overrides method in ComputeAPIProvider
        '''
        ...
        end_time_wait_resp = (datetime.utcnow()
                              .strftime(Constants.DATETIME_FORMAT))
        self.stacktachdb_provider.wait_for_launched_at(
            server_id,
            interval_time=self.interval_time,
            timeout=self.timeout)
    ...
    def insert_exists_event(self, exists_type, server,
        ...
        '''
        launched_at_datetime = string_to_datetime(launched_at)
        audit_period_beginning = launched_at_datetime.strftime(
        ...
        if exists_type == "immediate":
            ...
            bandwidth_usage = \
                json.dumps({'public': {'bw_in': 0,
                            ...
                            'private': {'bw_in': 0, 'bw_out': 0}})
            audit_period_ending = ((launched_at_datetime + timedelta(days=1))
