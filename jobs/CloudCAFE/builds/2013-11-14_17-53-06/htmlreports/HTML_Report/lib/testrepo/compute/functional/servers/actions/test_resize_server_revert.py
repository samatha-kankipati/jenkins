...
class ResizeServerUpRevertTests(ComputeFixture):
    ...
    def test_ram_and_disk_size_on_resize_up_server_revert(self):
        ...
        self.assertTrue(EqualityTools.are_sizes_equal(flavor.disk, remote_instance.get_disk_size_in_gb(self.config.compute_api.instance_disk_path), 0.5),
                        msg="Disk size %s after revert did not match %s" % (remote_instance.get_disk_size_in_gb(self.config.compute_api.instance_disk_path), flavor.disk))
