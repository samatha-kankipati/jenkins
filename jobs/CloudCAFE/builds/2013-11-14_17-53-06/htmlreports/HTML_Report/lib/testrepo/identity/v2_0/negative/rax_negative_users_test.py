...
class UsersNegativeTest(UserAdminFixture):
    ...
    def test_rax_list_users_by_email_test_domain_access(self):
        ...
        create_iden_admin = self.service_client.add_user(
                ...
                enabled=True,
                password=password)
        self.assertEqual(create_iden_admin.status_code, 201,
                         msg="Response for add identity admin is not 201.")
        # Delete user after test completion
        self.addCleanup(self.delete_user_permanently,
                        user_id=create_iden_admin.entity.id,
        ...
        create_user_admin = admin_client_test.add_user(
                ...
                domain_id=domain_id,
                password=password)
        self.assertEqual(create_user_admin.status_code, 201,
                         msg="Response for add user admin is not 201.")
        # Delete user after test completion
        self.addCleanup(self.delete_user_permanently,
                        user_id=create_user_admin.entity.id,
                        client=admin_client_test)
        ...
        create_sub_user = public_client_test.add_user(
                ...
                password=password)
        self.assertEqual(create_user_admin.status_code, 201,
                         msg="Response for add default user is not 201.")
        # Delete user after test completion
        self.addCleanup(self.delete_user_permanently,
                        user_id=create_sub_user.entity.id,
                        client=public_client_test)
    ...
    def test_rax_list_users_marker_limit_invalid_email(self):
        """
        Returns 200 when the routing flag is set as False.  Global Auth
        handles the request here.  But if routing flag is set as True,
        expected response is 501, because the request routed to
