...
class AdminTokenTest(IdentityAdminFixture):
    ...
    def test_validate_token(self):
        ...
        token_resp = self.admin_client.validate_token(token_id=token)
        self.assertEqual(
    ...
    def test_validate_token_belongs_to(self):
        ...
        token_resp = self.admin_client.validate_token(
            token_id=token,
            belongs_to=self.config.identity_api.tenant_id)
    ...
    def test_check_token(self):
        ...
        self.assertEqual(
            ...
            msg="Expected response 200 received response %s" %
                auth_resp.status_code)
        token = auth_resp.entity.token.id
        token_resp = self.admin_client.check_token(token_id=token)
        self.assertEqual(
            token_resp.status_code, 200,
            msg="Expected response 200 received response %s" %
                token_resp.status_code)
    ...
    def test_list_endpoints_for_token(self):
        ...
        self.assertEqual(
            ...
            msg="Expected response 200 received response %s" %
                auth_resp.status_code)
        token = auth_resp.entity.token.id
        token_resp = self.admin_client.list_endpoints_for_token(token_id=token)
        self.assertEqual(
            token_resp.status_code, 200,
            msg="Expected response 200 received response %s" %
                token_resp.status_code)
        self.assertTrue(
            token_resp.entity[0] is not None,
