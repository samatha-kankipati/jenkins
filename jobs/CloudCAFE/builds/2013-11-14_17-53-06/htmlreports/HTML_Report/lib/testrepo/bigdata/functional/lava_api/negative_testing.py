...
class ErrorMessageTest(LavaBaseFixture):
    ...
    def test_unauthorized_calls_admin_api(self):
        '''
        test all the calls for 403
        '''
        lava_admin_client = self.lava_provider.build_admin_client(
            self.config.auth.username,
            self.config.auth.password,
            self.config.auth.api_key,
            self.config.lava_api.ADMIN_API_URL(),
            self.config.auth.admin_auth_url(),
            self.config.auth.version,
            self.config.auth.admin_tenant_id,
            internal=False)

        # Get Clusters
        response = lava_admin_client.get_clusters()
        self.assertEquals(response.status_code,
                          403,
                          "{0}".format(response.content))

        # Get Limits
        response = lava_admin_client.get_limits()
        self.assertEquals(response.status_code,
                        403,
                        "{0}".format(response.content))

        # Set them to the required limits
        response = self.lava_client.Flavors.get_flavor_info(
            self.lava_provider.get_flavor_id("small")
        )
        flavor = response.entity
        response = lava_admin_client.set_limits(
        ...
        response = lava_admin_client.get_clusters_for_tenant(
            self.config.auth.tenant_id)
        self.assertEquals(response.status_code,
                          403,
                          "{0}".format(response.content))
        ...
        cluster = self.lava_provider.get_or_create_random_cluster_for_tenant(
            self.config.auth.tenant_id
        )
        response = lava_admin_client.set_cluster_to_error(cluster.id)
        self.assertEquals(response.status_code,
                          403,
                          "{0}".format(response.content))
    ...
    def load_tests(loader, standard_tests, pattern):
        suite = TestSuite()
        suite.addTest(LavaAPIAdminTest("test_unauthorized_calls_admin_api"))
        return suite
