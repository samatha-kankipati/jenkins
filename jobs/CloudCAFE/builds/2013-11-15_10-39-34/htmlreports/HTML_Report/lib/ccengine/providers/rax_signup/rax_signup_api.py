...
class RaxSignupProvider(BaseProvider):
    ...
    def get_default_client(
        ...
        return RaxSignupAPIClient(
            ...
            affiliate_id_source=self.config.rax_signup.affiliate_id_source)
    ...
    def get_ah_details_from_config(self):
        '''
        Helper method to get the value from config file and to be used in
        test cases
        '''
        resource_id = self.config.atom_hopper_events.ah_resource_id
        resource_name = self.config.atom_hopper_events.ah_resource_name
        return {'resource_id': resource_id,
                'resource_name': resource_name}

    def get_delay_time_in_seconds(self):
        '''
        Helper method to get the delay time before the get signup steps
        from the config file
        '''
        # As of now single parameter is used as there is one call
        # We can add in the dictionary
        wait_in_seconds = self.config.rax_signup.wait_in_seconds
        return {'wait_in_seconds': int(wait_in_seconds)}

    def get_affilitate_type_details(self):
        '''
        Helper method to get the value of affiliate code and type
        from the config file
        '''
        affiliate_code = self.config.rax_signup.affiliate_code
        affiliate_id_source = self.config.rax_signup.affiliate_id_source
        return {'affiliate_code': affiliate_code,
                'affiliate_id_source': affiliate_id_source}

    def get_welcome_email_templates(self):
        """
        Helper method to get the welcome_email_templates from config
        """
        welcome_mail_templates = self.config.rax_signup.welcome_email_templates
        return welcome_mail_templates.split(",\n")
