        ...
        if config.rackconnect.url is not None:
            ...
            if service is not None:
                ...
                self.provider_log.info(
                    'No endpoint url for rackconnect to make call')

        self.racknova_client = RackConnectApiClient(
            self.url, self.config.auth.username, self.config.auth.api_key,
            config.misc.serializer, config.misc.deserializer)
        self.rackcore_client = RackConnectApiClient(
            self.url, self.config.rackconnect.user,
            self.config.rackconnect.password, config.misc.serializer,
            config.misc.deserializer)
    
    def wait_for_rackconnect_deployed_status(
            self, server_id, status_to_wait_for):
        """
        ...
        automation_status = NovaRackConnectAutomationStatusTypes.PROGRESS
