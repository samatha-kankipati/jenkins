...
class AdminRolesTest(IdentityAdminFixture):
    ...
    def test_domain_crud_creator(self):
        ...
        api_response_admin = self.clientcreator.create_domain(name=self.name,
        ...
        api_response = self.clientcreator.list_all_domain()
        ...
        api_responsedomain = \
                self.clientcreator.list_domain_id(self.domain_id)
        self.assertTrue(api_responsedomain.ok)

        api_responsedomain = self.clientcreator.\
        ...
        time.sleep(20)
        api_responsecallback = self.clientcreator.\
        call_backUrl(callbackUrl)
        self.assertTrue(api_responsecallback.ok)

        api_response_cleanup = self.clientcreator.list_all_domain()
        ...
        if api_response_cleanup.entity.totalEntries != 0:
            for i in range(api_response_cleanup.entity.totalEntries):
                ...
                api_responsedel = self.clientcreator.\
