...
class TestQonosListServersOSSIFilterPositive(BaseImagesFixture):
    ...
    def test_enable_disabled_scheduled_images_w_larger_ret_list_server(self):
        ...
        6) Enable scheduled images using larger retention value for 1 server
        7) Verify that the response code is 200
        8) List servers with image-schedule set to true
        9) Verify that the response code is 200
        10) Verify that the response contains enabled server with
            image-schedule
        """
    ...
    def test_enable_disable_enable_sch_images_list_servers(self):
        ...
        4) Verify that the response code is 200
        5) Verify that the response doesn't have disabled server
        6) Enable scheduled images using valid retention value for 1 server
        7) Verify that the response code is 200
        8) List servers with image-schedule set to true
        9) Verify that the response code is 200
        10) Verify that the response contains enabled server with
