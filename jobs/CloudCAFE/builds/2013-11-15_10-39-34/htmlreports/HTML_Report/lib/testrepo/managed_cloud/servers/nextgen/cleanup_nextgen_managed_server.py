    ...
    def test_delete_all_server_on_account(self):
        ...
        if (len(list_servers) > 0):
            for server in list_servers:
                #Regex matches the pattern of the server names of the servers
                # created by the code in data_generators/managed_cloud
                if re.search(r"\w{3}-Managed-\w{4,8}-Server\d{6,13}",
                             server.name):
                    self.deleted_server = self.servers_client.delete_server(
                        server.id)
                    self.assertEqual(204, self.deleted_server.status_code,
                                     'The delete call #response was: {0}'
                                     .format(self.deleted_server.status_code))
            ...
            for image in list_images:
                if ("image" in image.name.lower()):
                    ...
                    self.assertEqual(204, self.deleted_image.status_code,
                                     'The delete call #response was: {0}'
                                     .format(self.deleted_image.status_code))
