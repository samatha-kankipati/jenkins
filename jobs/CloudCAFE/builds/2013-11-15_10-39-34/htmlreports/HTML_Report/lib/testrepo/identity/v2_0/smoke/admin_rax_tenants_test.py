...
class AdminTenantsTest(IdentityAdminFixture):
    ...
    def test_admin_get_tenants(self):
        ...
        get_tenants = self.public_client.get_tenants()

        self.assertIn(
            get_tenants.status_code, normal_response_codes,
    ...
    def test_admin_get_tenant_by_name(self):
        ...
        self.assertIn(
            tenant_info.status_code, normal_response_codes,
    ...
    def test_admin_get_tenant_by_id(self):
        ...
        tenant_info = self.admin_client.get_tenant_by_id(
            tenant_id=auth_resp.entity[0].id)
    ...
    def test_admin_list_roles_for_user_on_tenant(self):
        ...
        user_roles = self.admin_client.list_roles_for_user_on_tenant(
            tenant_id=self.config.identity_api.tenant_id,
            user_id=get_user.entity.id)

        self.assertIn(
            user_roles.status_code, normal_response_codes,
            msg='Admin list roles for user on tenant expected %s recieved'
                ' %s' % (normal_response_codes, user_roles.status_code))
