...
class ComputeIntegrationFixture(VolumesAPI_Fixture):
    @classmethod
    def setUpClass(cls):
    ...
    def explicit_attach_volume_to_server(
        ...
        assert resp.ok, \
            ("Volume attachment failed in explicit_attach_volume_to_server "
...
class BlockStorageLinuxIntegrationFixture(
    ...
    def setup_server_with_volume_attached(self, server=None, volume=None):
        ...
        testenv = LinuxBlockStorageTestEnv(
    ...
    def write_and_verify_data_on_mounted_volume(
            self, ssh_conn, mount_point, file_name=None, file_block_size=4096,
            file_block_count=25600):
        """Writes data to disk, verifies it's size, and returns an md5 hash
        of the written file.
        """
        ...
        current_hash = ssh_conn.get_file_md5hash(full_path_to_file)
        return current_hash

    def verify_md5_hash(self, ssh_conn, filepath, original_hash):
        # Verify file md5 sum
        current_hash = ssh_conn.get_file_md5hash(filepath)
        self.fixture_log.debug("ORIGINAL FILE HASH: {0}".format(current_hash))
        self.fixture_log.debug("NEW FILE HASH: {0}".format(current_hash))
        assert current_hash == original_hash, (
            "Original and restored file md5 hashes did not match: "
            "{0} != {1}".format(original_hash, current_hash))
