...
class AuthenticationNegativeTest(UserAdminFixture):
    ...
    def test_base_auth_tenant_id_and_token_false_token(self):
        ...
        self.assertEqual(
            ...
            msg=("Expected response 401 but received {0}".format(
                auth_resp.status_code)))
        self.assertTrue(
            'Token not authenticated' in auth_resp.content,
            msg=("Expecting 'Token not authenticated' but received {0}".format(
                auth_resp.content)))
