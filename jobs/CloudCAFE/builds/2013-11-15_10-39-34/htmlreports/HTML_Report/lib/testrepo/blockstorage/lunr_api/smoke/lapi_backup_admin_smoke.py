...
class LunrBackupAdminAPISmokeTest(LunrAPIFixture):
    ...
    def setUpClass(cls):
        ...
        for backup in cls.expected_backups:
            resp = cls.LunrAPIProvider.wait_for_volume_backup_status(
                backup.id, LunrBackupStatusTypes.READY, cls.user_client)
            if not resp.ok:
                raise Exception(
                    "Snapshot in setup failed to reach the READY status")
    ...
    def test_delete_backup(self):
        timeout = 10
        for expected_backup in self.expected_backups:
            expected_status = LunrBackupStatusTypes.DELETING
            resp = self.admin_client.Backups.delete(expected_backup.id)
            actual_deleted_backup = BackupResponse(**json.loads(resp.content))

            self.assertTrue(
                resp.ok, "Delete Backup API call returned {0}, "
                "expected 2XX".format(resp.status_code))

            wait_status = self.LunrAPIProvider.wait_for_volume_backup_status(
                actual_deleted_backup.id, expected_status,
                user_client=self.user_client, timeout=timeout)

            self.assertTrue(
                wait_status.ok,
                "Backup {0}did not enter the DELETING status within {1} "
                "seconds of issuing a DELETE request.  Last observed status "
                "was {2}".format(
                    timeout, wait_status.entity.status,
                    actual_deleted_backup.id))
