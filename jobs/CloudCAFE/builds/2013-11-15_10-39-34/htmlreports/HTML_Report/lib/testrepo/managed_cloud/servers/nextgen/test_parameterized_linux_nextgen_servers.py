...
class ManagedLinuxNextGenServersTest(ComputeFixture,
    ...
    def setUpClass(cls):
        ...
        cls.srv_location = cls.config.compute_api.region
    ...
    def get_remote_client(self):
        ...
        public_address = self.compute_provider.get_public_ip_address(
            self.created_server)
        if (hasattr(self.created_server, public_address)):
    ...
    def get_su_remote_client(self):
        su_remote_client = ""
        public_address = self.compute_provider.get_public_ip_address(
            self.created_server)
        if (hasattr(self.created_server, public_address)):
            su_remote_client = self.compute_provider.get_remote_instance_client(
                self.created_server, username="root",
    ...
    def test_rackspace_monitoring_agent_running(self):
        ...
        self.assertTrue("rackspace-monitoring-agent" in remote_client
                        .remote_process_grep("rackspace-monitoring-agent"),
                        msg="Monitoring Agent not running on the server")

    def test_mckick_complete(self):
        remote_client = self.get_su_remote_client()
        status = remote_client.is_file_present(
    ...
    def test_apache_open_port_80(self):
        ...
        status = ssh_client.exec_command(
            'iptables-save | grep dport.80.*ACCEPT')
    ...
    def test_apache_open_port_443(self):
        ...
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command(
            'iptables-save | grep dport.443.*ACCEPT')
    ...
    def test_fail2ban_firewall(self):
        remote_client = self.get_su_remote_client()
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command('iptables-save | grep fail2ban')
        self.assertIsNotNone(status)
        self.assertTrue(status, msg="fail2ban firewall not setup")
        status = ssh_client.exec_command('iptables-save | grep -c "dport 22"')
        self.assertIsNotNone(status)
        self.assertEquals(status, "2\n", msg="Port 22 in firewall not setup")
    ...
    def test_show_processlist(self):
        ...
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command(
            'mysqladmin pr | grep "show processlist"')
        self.assertIsNotNone(status)
        self.assertTrue(status)
    ...
    def test_holland_bk(self):
        remote_client = self.get_su_remote_client()
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command('holland bk')
        self.assertIsNotNone(status)
    ...
    def test_root_muninpass(self):
        ...
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command(
            'grep ^serverinfo.. /root/.muninpass')
        self.assertIsNotNone(status)
        self.assertTrue(status)
        self.assertTrue("serverinfo" in status,
                        msg="munin failure")
    ...
    def test_root_phpmyadminpass(self):
        ...
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command(
            'grep ^serverinfo.. /root/.phpmyadminpass')
        self.assertIsNotNone(status)
        self.assertTrue(status)
        self.assertTrue("serverinfo" in status,
    ...
    def test_munin_running(self):
        remote_client = self.get_su_remote_client()
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command("ps xo comm | grep munin")
        self.assertIsNotNone(status)
        self.assertTrue(status)
    ...
    def test_postfix_config_file(self):
        remote_client = self.get_remote_client()
        status = remote_client.is_file_present("/etc/postfix/main.cf")
        self.assertTrue(status,
                        msg="Postfix config file not present")

    def test_sendmail_file(self):
        remote_client = self.get_remote_client()
        status = remote_client.is_file_present(
            "/usr/sbin/sendmail")
        self.assertTrue(status,
                        msg="Sendmail file not present")

    def test_mailgun_configured(self):
        remote_client = self.get_remote_client()
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command(
            'grep "relayhost = smtp.mailgun.org" /etc/postfix/main.cf')
        self.assertTrue("smtp.mailgun.org" in status,
                        msg="Mailgun not configured in postfix")

    def test_mailgun_sendmail(self):
        remote_client = self.get_remote_client()
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command(
            'echo test | /usr/sbin/sendmail test@rackspace.com')
        self.assertEqual(status, '', msg="Mail was not sent")

    def test_listen_port_25(self):
        remote_client = self.get_su_remote_client()
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command('ss -ln | grep :25')
        self.assertTrue(':25' in status, msg="Port 25 is opened for SMTP")

    def test_listen_port_587(self):
        remote_client = self.get_remote_client()
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command('ss -ln | grep :587')
        self.assertEquals(status, '', msg="Port 587 is opened for TLS")

    def test_listen_port_465(self):
        remote_client = self.get_remote_client()
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command('ss -ln | grep :465')
        self.assertEquals(status, '', msg="Port 465 is opened for SSL")

    def test_holland_file(self):
        remote_client = self.get_remote_client()
        status = remote_client.is_file_present(
            "/usr/lib/rackspace-monitoring-agent/plugins/holland_monitor.py")
        self.assertTrue(status, msg="Holland file not present")

    def test_holland_run(self):
        remote_client = self.get_su_remote_client()
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command(
            'ls /var/lib/mysqlbackup/default/ | wc -l')
        file_count = int(status)
        ssh_client.exec_command('holland bk')
        status = ssh_client.exec_command(
            'ls /var/lib/mysqlbackup/default/ | wc -l')
        after_backup_count = int(status)
        self.assertEqual(after_backup_count, file_count + 1,
                         msg="Backup did not complete")

    def test_server_timezone(self):
        remote_client = self.get_remote_client()
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command('date +%z')
        time_zone = {'ORD': '-0500\n', 'IAD': '-0400\n', 'DFW': '-0500\n',
                     'SYD': '+1000\n', 'LON': '+0100\n'}
        location = self.srv_location
        self.assertEquals(status, time_zone[location],
                          msg="Server not in DC timezone")

    def test_backup_configured(self):
        remote_client = self.get_su_remote_client()
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command(
            'grep "RsaKeyPair" /etc/driveclient/bootstrap.json')
        self.assertTrue("RsaKeyPair" in status,
                        msg="Backup agent is not installed")

    def test_backupagent_bootstrap(self):
        remote_client = self.get_su_remote_client()
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command(
            'sudo tail -10 /var/log/driveclient.log;'
            ' grep RsaKeyPair /etc/driveclient/bootstrap.json')
        self.assertTrue("RsaKeyPair" in status,
                        msg="BackupAgent bootstrap issues")

    def test_monitoring_connected(self):
        remote_client = self.get_remote_client()
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command(
            'grep -c "> Connected" /var/log/rackspace-monitoring-agent.log')
        self.assertEquals(status, "3\n",
                          msg="Monitoring not connected")

    def test_monitoring_authenticated(self):
        remote_client = self.get_remote_client()
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command(
            'grep -c "Connection has been authenticated" '
            '/var/log/rackspace-monitoring-agent.log')
        self.assertEquals(status, "3\n",
                          msg="Monitoring not connected")

    def test_monitoring_check_created(self):
        remote_client = self.get_su_remote_client()
        ssh_client = remote_client.ssh_client
        status = ssh_client.exec_command(
            'grep -c "Created Check: agent.filesystem" '
            '/var/log/rackspace-monitoring-agent.log')
        self.assertTrue(status)
        self.assertGreaterEqual(status, 1,
                                msg="Filesystem monitoring check not setup")
        status = ssh_client.exec_command(
            'grep -c "Created Check: agent.cpu" '
            '/var/log/rackspace-monitoring-agent.log')
        self.assertTrue(status)
        self.assertGreaterEqual(status, 1,
                                msg="CPU monitoring check not setup")
        status = ssh_client.exec_command(
            'grep -c "Created Check: agent.memory" '
            '/var/log/rackspace-monitoring-agent.log')
        self.assertTrue(status)
        self.assertGreaterEqual(status, 1,
                                msg="Memory monitoring check not setup")

    def test_yumcron_file(self):
        remote_client = self.get_su_remote_client()
        ssh_client = remote_client.ssh_client
        yum_installed = ssh_client.exec_command('yum --version')
        if ('Installed' in yum_installed):
            status = remote_client.is_file_present("/var/lock/subsys/yum-cron")
            self.assertTrue(status, msg="YUM cron file not present")

    def test_yumcron_activated(self):
        remote_client = self.get_su_remote_client()
        ssh_client = remote_client.ssh_client
        yum_installed = ssh_client.exec_command('yum --version')
        if ('Installed' in yum_installed):
            status = remote_client\
                .is_file_present("/etc/rc.d/rc3.d/S50yum-cron")
            self.assertTrue(status, msg="YUM cron file not activated")
    ...
    def test_backup_config(self):
        ...
        self.assertEquals(str(agent_config['NotifyFailure']),
                          'True', msg='Backup failure notification not set')
