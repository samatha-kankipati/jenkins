...
from ccengine.common.exceptions.compute \
    import BadRequest, Forbidden, InternalServerError, ItemNotFound
...
class TestQonosUpdateScheduleNegative(BaseImagesFixture):
    ...
    def test_update_schedule_method_mismatch(self):
        ...
        with self.assertRaises(ItemNotFound):
            self.images_provider.schedules_client. \
                update_schedule(sch.id, requestslib_kwargs={'method': method})
    ...
    def test_update_schedule_request_incorrect_parameter(self):
        ...
        bad_param = {'schedule': {'tenant': tenant, 'action': action,
                                  'schedule_id': sch.id}}

        with self.assertRaises(BadRequest):
            self.images_provider.schedules_client. \
                update_schedule(sch.id, requestslib_kwargs={'data': bad_param})
    ...
    def test_update_schedule_using_numbers_for_action(self):
        ...
        self.images_provider.schedules_client.update_schedule(
            id=sch_obj.entity.id, tenant=tenant, action=alt_action)
    ...
    def test_update_schedule_for_next_run(self):
        ...
        3) Verify that the schedule is updated by checking response code
        """
