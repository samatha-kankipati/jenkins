...
from ccengine.common.data_generators.managed_cloud. \
    nextgen_rebuild_windows_servers_generator \
    import ManagedCloudWindowsNextGenRebuildDataGenerator
from unittest2.suite import TestSuite


def load_tests(loader, standard_tests, pattern):
    suite = TestSuite()
    smoke_data_gen = ManagedCloudWindowsNextGenRebuildDataGenerator()
    custom_loader = BaseParameterizedLoader(smoke_data_gen)
    custom_loader.addTest(ManagedWindowsNextGenServersTest(
        "test_managed_server_rebuild"))
    custom_loader.addTest(ManagedWindowsNextGenServersTest(
