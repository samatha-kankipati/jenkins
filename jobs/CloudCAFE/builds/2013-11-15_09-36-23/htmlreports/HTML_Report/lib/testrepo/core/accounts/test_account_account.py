...
class TestAccountAccount(CoreFixture):
    ...
    def setUpClass(cls):
        ...
        cls.datacenter_id = cls.config.core.datacenter_id
        cls.contract_id = cls.config.core.contract_id

    @attr(type='positive', suite='smoke')
    ...
    @attr(suite='smoke', type='positive')
    def test_add_new_server_to_contract(self):
        """
        @summary: Verify user can add a new server to a given contract by
        selecting a data center
        """
        response = self.contract_client.add_new_server(self.contract_id,
                                                       self.datacenter_id)
        self.assertEqual(200, response.status_code,
                         "Status code {0} is not same as 200"
                         .format(response.status_code))

    @attr(suite='smoke', type='negative')
    def test_add_new_server_to_invalid_contract(self):
        """
        @summary: Verify user gets an exception by adding a valid data center
        to an invalid contract
        """
        contract_id = 1233546456
        response = self.contract_client.add_new_server(contract_id,
                                                       self.datacenter_id)
    ...
    @attr(suite='smoke', type='negative')
    def test_add_new_server_invalid_data_center(self):
        """
        @summary: Verify user will get an error message while trying to add
        server to a given contract  with an invalid data center
        """
        datacenter_id = 122142745
        response = self.contract_client.add_new_server(self.contract_id,
                                                       datacenter_id)
    ...
    def test_verify_account_details_using_MonitoringURLLogWhere(self):
        """
        @summary: get all the attributes of a account using MonitoringURLLogWhere
        as loadargs and verify the user can get all the attributes of an account
        """
        where_condition = WhereEquals("id", self.account_id)
        attributes = ["account_coordinator", "account_exec",
                      "account_level_domains", "account_type_id",
                      "active_mbu_devices", "additional_bandwidth",
                      "all_employees", "all_notes", "am_queue", "ar_specialist",
                      ...
                      "support_queue", "total_bandwidth", "number", "id"]
        response = self.account_client.\
            get_account_attributes(where_condition, attributes=attributes)
        self.assertEqual(response.status_code, 200, "Actual response code is{0}"
                         .format(response.status_code))
