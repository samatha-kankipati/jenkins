...
class ComputeAPIProvider(BaseProvider):
    ...
    def __init__(self, config, logger=None):
        ...
        if config.compute_api.bypass_url:
            self.compute_public_url = config.compute_api.bypass_url
    ...
    def wait_for_image_to_be_deleted(self, image_id):
        ...
        time_waited = 0
        interval_time = self.config.compute_api.build_interval
        # Added try except around delete image call since many
        # RBAC tests rely on this method to delete an image
        try:
            image_response = self.images_client.delete_image(image_id)
        except ItemNotFound as err:
            self.provider_log.info(
                "{0}: Tried to delete image, perhaps already deleted? "
                "Image id: {1}".format(err, image_id))
        while (True):
            try:
                image_obj = self.images_client.get_image(image_id).entity
            except ItemNotFound as err:
                self.provider_log.info(
                    "{0}: Queried image was not found during delete. "
                    "Image id: {0}".format(err, image_id))
            else:
                if image_obj.status.lower() == ImageStatus.ERROR.lower():
                    raise BuildErrorException(
                        "Image entered Error state while deleting. "
            ...
            if time_waited > self.config.compute_api.server_status_timeout:
                raise TimeoutException(
                    "Timed out while deleting. Image id: {0}".format(image_id))
            time.sleep(interval_time)
            time_waited += interval_time
