...
class AuthClient(BaseRESTClient):
    ...
    def auth_flow(self):
        """ Authentication for Flow API """
        headers = {'Content-Type': 'application/json',
                   'Accept': 'application/json'}
        full_url = '{0}/login.json'.format(self.url)
        params = 'username={0}&password={1}'.format(
                        self.username, self.password)
        auth_token = self.request(
            'POST', full_url, data=params, headers=headers)
        return auth_token
