...
from ccengine.clients.bigdata.lava_api_client import LavaAPIClient as\
    _LavaAPIClient
from ccengine.clients.bigdata.lava_api_admin_client import LavaAdminApiClient
...
class LavaAPIProvider(BaseProvider):
    ...
    def authenticate_with_ttl(self,
                              auth_url,
                              auth_version,
                              username,
                              password,
        ...
        auth_client = _AuthClient(
            ...
            password)
        if internal:
            response = auth_client.auth_2_0_internal()
        else:
        ...
        # Change the hard coded token to password based auth
        # and get the username/pass from the config
        if ttl_hours < 2:
            identity_admin_client = IdentityAdminClient(
                url=self.config.auth.base_url,
                serialize_format='json',
                auth_token="my-simple-demo-token"
            )
            identity_admin_client.revoke_token(auth_data.token.id)
            if internal:
                response = auth_client.auth_2_0_internal()
            else:
                response = auth_client.auth_2_0()
    ...
    def build_lava_client(self,
                          ...
                          auth_version,
                          tenant_id):
        auth_data = self.authenticate_with_ttl(auth_url,
                                               auth_version,
                                               username,
                                               password,
                                               api_key,
                                               internal=False)
        return _LavaAPIClient(api_url, auth_data.token.id, tenant_id)

    def build_admin_client(self, username, password, api_key, admin_api_url,
                           admin_auth_url, admin_auth_version, admin_tenant_id,
                           internal=True):
        admin_auth_data = self.authenticate_with_ttl(
            admin_auth_url,
            admin_auth_version,
            username,
            password,
            api_key,
            internal=internal)
        return LavaAdminApiClient(
            admin_api_url,
            admin_auth_data.token.id,
            admin_tenant_id
        )
    ...
    def add_tenant(self, tenant_id):
        identity_admin_client = IdentityAdminClient(
            ...
            auth_token="my-simple-demo-token")
        '''
        Need to get the end point id instead
        '''
        response = identity_admin_client.add_end_point_for_tenant_id(
            tenant_id,
            "******")
    ...
    def remove_tenant(self, tenant_id):
        ...
        Need to get the end point id instead
        '''
        response = identity_admin_client.remove_end_points_for_tenant_id(
            tenant_id,
            "******")

    def check_if_valid_tenant(self, tenant_id):
        response = self.lava_admin_client.get_tenants(tenant_id)
        response_json = json.loads(response.content)
        returned_tenant_id = response_json['tenantId']
        return returned_tenant_id == tenant_id

    def set_lava_client(self, lava_api_client):
        if lava_api_client is not None:
            self.lava_client = lava_api_client
            
    def retreive_admin_cred(self, variable_name):
        '''
        Admin credentials are an SSO account that I cannot check into github.
        So we let jenkins set the environment variables and let the tests pick
        it up from there.
        '''
        if self.config.auth.get(variable_name) is not None:
            return self.config.auth.get(variable_name)
        elif "lava_{0}".format(variable_name) in os.environ.keys():
            return os.environ["lava_{0}".format(variable_name)]
    
    def set_admin_credentials(self):
        self.admin_username = self.retreive_admin_cred("admin_username")
        self.admin_password = self.retreive_admin_cred("admin_password")
        self.admin_api_key = self.retreive_admin_cred("admin_api_key")
        self.admin_auth_url = self.retreive_admin_cred("admin_auth_url")
        self.admin_auth_version = self.retreive_admin_cred("admin_auth_version")
        self.admin_tenant_id = self.retreive_admin_cred("admin_tenant_id")
            
    def __init__(self, config):
        super(LavaAPIProvider, self).__init__()
        self.config = config
        self.lava_client = self.build_lava_client(
            self.config.auth.username,
            self.config.auth.password,
            self.config.auth.api_key,
            self.config.lava_api.BASE_URL,
            self.config.auth.base_url,
            self.config.auth.version,
            self.config.auth.tenant_id)
        self.set_admin_credentials()
        self.lava_admin_client = self.build_admin_client(
            self.admin_username,
            self.admin_password,
            self.admin_api_key,
            self.config.lava_api.ADMIN_API_URL,
            self.admin_auth_url,
            self.admin_auth_version,
    ...
    def create_cluster(self, name=None, count=1, type=LavaClusterTypes.HADOOP_HDP,
                       flavor_name="small", timeout=500):
        '''
    ...
    def concurrent_cluster_create(self, flavor_id):
        ...
        for cluster in cluster_list:
            ...
            wait_result = self.wait_for_cluster_build(temp_clus_id)
    ...
    def wait_for_cluster_build(self, cluster_id, resize = False,
                               timeout=500):
        if resize:
            status = [_LavaClusterStatusTypes.UPDATING,
                      _LavaClusterStatusTypes.ACTIVE]
        else:
            status = [_LavaClusterStatusTypes.BUILDING,
                      _LavaClusterStatusTypes.CONFIGURING,
                      _LavaClusterStatusTypes.ACTIVE]        
        self.provider_log.info(("Waiting for cluster id: {0} to reach"
                                " status {1} in {2} seconds").format(
                               cluster_id, _LavaClusterStatusTypes.ACTIVE,
                               timeout))
            ...
            self.provider_log.debug(
                ("Waiting for cluster #{0} Status #{1}, current Status"
                 " is #{2}, Seconds remaining: #{3} . .").format(
                     ...
                     current_status,
                     max_time - time.time()))
            if current_status == _LavaClusterStatusTypes.ACTIVE:
                self.provider_log.info(
                    "Cluster {0} returned status {1}".format(
                        cluster_id,
                        _LavaClusterStatusTypes.ACTIVE))
                ...
                self.provider_log.info(
                    "Cluster {0} returned status {1}".format(
                        cluster_id,
                        _LavaClusterStatusTypes.ACTIVE))
                return  provider_action_result, "Error status"
            else:
                ...
                '''
                Open Bugs Redmine - #337, 339
                '''
                if progress is None and resize is False:
                ...
                #    return provider_action_result,
                #    "Invalid Progess {0}".format(
                #        progress
                #    )
                time.sleep(
                    int(
                        self.config.lava_api.CLUSTER_SLEEP_INTERVAL))
        timeout_message = "Unable to reach {0} status due to timeout".format(
            _LavaClusterStatusTypes.ACTIVE
        )
    ...
    def create_ssh_connector(self, node, port=22):
        ...
        key_filenames.append(
            "/tmp/{0}".format(self.config.lava_api.PROFILE_KEY_FILE_NAME))
        self.provider_log.info('Creating SSH Session to test server')
        ssh_connector = SSHConnector(node.ip, username, password,
                                     360, port, key_filenames=key_filenames)
        if ssh_connector.test_connection_auth():
            return ssh_connector

    def delete_clusters(self, clusters_to_delete=None, only_active=True):
        '''
        @summary: Iterate through list of clusters and delete any
                  with status "active". If list not provided deletes all
                  clusters
        ...
        if not clusters_to_delete:
            clusters = self.lava_client.Clusters.list().entity
        else:
            clusters = clusters_to_delete
        for cluster in clusters:
            if only_active and cluster.status == _LavaClusterStatusTypes.ACTIVE:
                self.provider_log.info("Tear Down: Deleting cluster '%s'"
                                       % cluster.id)
                self.lava_client.Clusters.delete(cluster.id)
            else:
                self.provider_log.info("Tear Down: Deleting cluster '%s'"
                                       % cluster.id)
                self.lava_client.Clusters.delete(cluster.id)

    def get_active_cluster_using_admin(self):
        clusters = self.lava_admin_client.get_clusters().entity
        for cluster in clusters:
            if cluster.status == _LavaClusterStatusTypes.ACTIVE:
                return cluster

    def get_or_build_active_cluster(self):
        clusters = self.lava_client.Clusters.list().entity
        for cluster in clusters:
            if cluster.status == _LavaClusterStatusTypes.ACTIVE:
                return cluster
        stamp = time.mktime(time.localtime())
        rand_name = "lava_test_{0}".format(int(stamp))
        pac, msg = self.create_cluster(rand_name)
        return pac.response.entity