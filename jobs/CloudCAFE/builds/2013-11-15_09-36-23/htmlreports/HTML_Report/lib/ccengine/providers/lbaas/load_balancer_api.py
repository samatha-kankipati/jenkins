...
class LoadBalancersProvider(BaseProvider):
    ...
    def create_active_load_balancer(self, retry_on_error=True, num_tries=0,
        ...
        if 'nodes' not in kwargs and not nodeless:
    ...
    def create_n_load_balancers(self, n, wait_for_active=False,
                                retry_on_error=True, nodeless=False, **kwargs):
        if 'name' not in kwargs:
            kwargs['name'] = datagen.random_string('cc_lb')
        if 'port' not in kwargs:
            kwargs['port'] = '80'
    ...
    def generate_bandwidth_out(self, ip, path='/medium.iso', output=False):
        self.provider_log.info("Generating outgoing normal bandwidth to {0}"
                               "...".format(ip))
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.connect((ip, 80))
            size_in = sock.send("GET {0} HTTP/1.0\r\n\r\n".format(path))

        # Many types of socket errors, all types should be caught
        except Exception as err:
            self.provider_log.error("Unable to generate HTTP traffic: "
                                    "{err}".format(err=err))
            return (0, 0, 0) if output else (0, 0)
        ...
        self.provider_log.info("Generated {0} bytes of outgoing bandwidth "
                               "to {0}...".format(size_out, ip))
        result = (size_in, size_out)
        if output:
            result += (msg,)
        return result

    def generate_bandwidth_in(self, ip, output=False):
        self.provider_log.info("Generating incoming bandwidth to {0}"
                               "...".format(ip))
        msg_body = '1011' * 1000000
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.connect((ip, 80))
            size_in = sock.send("POST / HTTP/1.0\r\nContent-Length: {0}\r\n"
                                "Content-Type: application/text\r\n\r\n"
                                "{1}".format(len(msg_body), msg_body))
        ...
        while len(chunk) != 0:
            ...
            resp_msg += chunk
        sock.close()
        size_out = len(resp_msg)
        self.provider_log.info("Generated {0} bytes of incoming bandwidth "
                               "to {0}...".format(size_in, ip))
        results = (size_in, size_out)
        if output:
            results += (resp_msg,)
        return results

    def generate_ssl_bandwidth_out(self, ip, path='/small.iso', output=False):
        ...
        # Many types of socket errors, all types should be caught
            ...
            self.provider_log.error("Unable to generate SSL traffic: "
                                    "{err}".format(err=err))
            return (0, 0, 0) if output else (0, 0)
        ...
        self.provider_log.info("Generated {0} bytes of outgoing SSL bandwidth "
                               "to {0}...".format(size_out, ip))
        if size_out < 1000:
            self.provider_log.info("Outgoing SSL bandwidth is small, this is "
                                   "what was returned: {0}".format(msg))
    ...
    def generate_ssl_bandwidth_in(self, ip, output=False):
