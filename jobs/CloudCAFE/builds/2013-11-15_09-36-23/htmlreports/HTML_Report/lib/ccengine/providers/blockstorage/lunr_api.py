...
class LunrAPIProvider(BaseProvider):
    ...
    def wait_for_volume_backup_status(
            self, backup_id, expected_status, user_client, timeout=120,
            wait_preiod=10):
        ...
        while time.time() < end:
            self.provider_log.info(
                "Waiting for volume backup {0} to become {1}. . .".format(
                    backup_id, expected_status))
            api_response = user_client.Backups.get_info(backup_id)
            provider_action_result.response = api_response

            #Special case if backup disappears and we where waiting on delete
            if api_response.status_code == 404 and expected_status in \
                    ['DELETED', 'DELETING']:
                provider_action_result.ok = True
                break
                ...
                self.provider_log.error(
                    "A GET to backup {0} resulted in an HTTP {1}".format(
                        backup_id, api_response.status_code))
                continue
            elif api_response.ok:
                #Get the status from the backup info response
                backup_domain_object = _BackupDomainObject(
                ...
                if current_status == expected_status:
                    self.provider_log.info(
                        "Expected volume backup status '{0}' observed for backup "
                        "{1}".format(expected_status, backup_id))
                    provider_action_result.ok = True
                    break
                else:
                    self.provider_log.info(
                        "Current status of volume backup {0} is '{1}'.  Continuing"
                        " wait for '{2}' status for {3} more seconds".format(
                            backup_id, current_status, expected_status,
                            (end - time.time())))

            time.sleep(wait_preiod)
        else:
            self.provider_log.error(
                "wait_for_volume_backup_status timed out after {0} seconds."
                "  The backup {1} was never observed to have entered the {2}"
                " status".format(timeout, backup_id, expected_status))
            provider_action_result.ok = False
    ...
    def initiate_delete_for_user_backups(self, account_client, timeout=30):
        '''@summary Issues a DELETE for every backup on a user's account, but
        doesn't wait for the eventual DELETED status.
        @type account_client: L{LunrAPIClient}
        ...
        failures = failures or not api_response.ok
        if not api_response.ok:
            self.provider_log.error(
                'API returned {0}'.format(api_response.status_code))

        backups = self.convert_json_to_domain_object_list(
            json.loads(api_response.content), _BackupDomainObject)

        for backup in backups:
            self.provider_log.info("Deleting Backup: {0}".format(backup))
            api_response = account_client.Backups.delete(backup.id)
            failures = failures or not api_response.ok
            if not api_response.ok:
                self.provider_log.warning(
                    "Call to Delete Backup {0} failed".format(backup))

        for backup in backups:
            if backup.status == _LunrBackupStatusTypes.DELETING:
                continue
            else:
                wait_status = self.wait_for_volume_backup_status(
                    backup.id, _LunrBackupStatusTypes.DELETING,
                    user_client=account_client, timeout=timeout)
                failures = failures or not wait_status.ok

        return failures

    def delete_user_backups(self, account_client):
        '''@summary Issues a DELETE for every backup on a user's account, and
        waits for those backups to enter the DELETING and eventually the
        DELETED state.
        @type account_client: L{LunrAPIClient}
        @return: Whether all backups were successfully deleted
        @rtype: C{bool}
