...
class SessionPersistenceSmokeTests(LoadBalancersSmokeFixture):
    ...
    def tearDown(self):
        # Since the tests are modifying the protocol on the global
        # instance of the LB, reset the LB protocol to HTTP after
        # each test.
        protocol = 'HTTP'
        resp = self.client.update_load_balancer(self.lb.id, protocol=protocol)
