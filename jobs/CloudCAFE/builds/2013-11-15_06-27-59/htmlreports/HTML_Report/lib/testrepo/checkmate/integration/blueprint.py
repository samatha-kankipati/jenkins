...
class BlueprintTest(CheckmateFixture):
    ...
    def test_sim_bp(self):
        ''' Testing Simulated Blueprint '''

        # TODO: Test deployments without embedding them
        api_response = self.checkmate_provider.client.simulate(deployment=wordpress)
        
        j = json.loads(api_response.content)
        print (j['blueprint']['description'])
        self.assertTrue(api_response.ok, "Did not get ok from server: \
            '%s' and status code '%s': \n '%s'" \
