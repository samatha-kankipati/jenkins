...
class RaxSignupAPI_BaseFixture(BaseTestFixture):
    ...
    def get_signup_request_dict(cls, **kwargs):
        ...
        if kwargs.get('affiliate_code_and_type'):
            affiliate_code_and_type = {'code': 'My Affiliate',
                                       'type_': 'BRC'}
            affiliate_code_and_type = override_dict(affiliate_code_and_type,
                                                    'affiliate_code_and_type')
        else:
            affiliate_code_and_type = None
    ...
    def validate_contract_entity(self, api_response=None, country_code='US',
                                 cloud_type='US', business_type='CONSUMER',
                                 contract_entity=None,
                                 is_consolidated_invoice=False):
        """
        ...
        apac_countries = ['BD', 'KH', 'CN', 'HK', 'IN', 'ID', 'JP', 'KP', 'KR',
                          ...
                          'NP', 'TM', 'UZ']
        anz_countries = ['AU', 'NZ']
        ch_countries = apac_countries + anz_countries
        ...
        self.assertIsNotNone(re.search('CONTRACT_ENTITY', api_response1),
                             'Contract Entity call is not done')
        ...
        contract_entity_returned = contract_entity
        ...
        if cloud_type == 'US' and not is_consolidated_invoice:
    ...
    def validate_atom_hopper_call(self, api_response=None, resource_id=None,
                                  resource_name=None, sites_purchased=True):
        """
        Helper method to check whether the atom hopper call is made and
        to check the value of resource id and resource name
        """
        # Pause for the call to be done
        self.pause_in_seconds()
        ...
        account_id = api_response.entity.id_
        ...
        get_signup_resp = self.client.get_a_signup(signup_id).content
        self.assertIsNotNone(re.search(status_regex, get_signup_resp),
                             "Status of SignUp is not COMPLETE")

        # To check that atom hopper is not called unless Cloud Sites is
        # purchased
        if not sites_purchased:
            self.assertIsNone(re.search(atom_hopper_regex, steps_response),
                              "Atom Hopper step is present")
            return

        # To check whether the signup id is posted to the atom feed
        self.assertIsNotNone(re.search(atom_hopper_regex, steps_response),
                             "Atom Hopper step is not present")
        resource_id_regex = "resourceId=\\\"{0}\\\"".format(resource_id)
        resource_name_regex = "resourceName=\\\"{0}\\\"".format(resource_name)
        self.assertIn(resource_id_regex, steps_response,
                      "Resource id not as expected")
        self.assertIn(resource_name_regex, steps_response,
                      "Resource id not as expected")
    ...
    def welcome_email_assertions(self, api_response, default_region=None,
                                 geography='US', service_level='MANAGED'):
        """
        Helper method to assert the welcome email step
        """
        self.pause_in_seconds()
        welcome_mail_templates = self.get_welcome_email_templates()
        # To get the signupId
        signup_id = self.get_marker(api_response)

        # To check the steps for Welcome Email call is not done
        steps_response = (self.client.signup_steps(signup_id)).content

        # Checking the steps for Welcome Mail is been sent
        welcome_email_step_check_regex = "CLOUD_WELCOME_EMAIL COMPLETE"
        welcome_email_step_regex = "CLOUD_WELCOME_EMAIL COMPLETE(.+)?Success"
        self.assertIn(welcome_email_step_check_regex, steps_response,
                      "Welcome Email Step was not made")

        welcome_email_step = re.search(welcome_email_step_regex,
                                       steps_response).group(1)
        template_picked = None
        # Deciding the template
        if default_region is not None:
            for template_picked in welcome_mail_templates:
                if default_region.lower() in template_picked:
                    break
        else:
            for template_picked in welcome_mail_templates:
                if (geography+"."+service_level).lower() in template_picked:
                    break

        #Assert whether the right template is been picked
        self.assertIn(template_picked, welcome_email_step,
                      "The expected template is {0}".format(template_picked))
    ...
    def pause_in_seconds(method, seconds=None):
        if seconds is None:
            time.sleep(method.get_wait_in_seconds())
        else:
            time.sleep(seconds)

    def assert_time_format_for_get_calls(method, api_response):
        date_format_regex = "\"date\":\"....-..-..T..:..:..\....Z\","
        date_in_response = re.search("date\":\"(.+)\"?,\"duration",
                                     api_response.content).group(1)
        method.assertIsNotNone((re.search(date_format_regex,
                                api_response.content)),
                               "The date format is not as Expected. The date\
                               returned is {0}".format(date_in_response))

    @classmethod
    def get_affiliate_type(cls):
        """
        Helper Method to get affiliate_type from config
        """
        affiliate_type = cls.provider.\
            get_affilitate_type_details()['affiliate_code']
        return affiliate_type

    @classmethod
    def get_welcome_email_templates(cls):
        """Helper method to get template titles from config"""
        return cls.provider.get_welcome_email_templates()

    def get_affiliate_code(self):
        """
        Helper Method to get affiliate_code from the get call
        """
        get_transaction_response = self.client.get_transaction_id()
        location = get_transaction_response.history[0].headers['location']
        affiliate_code = re.search('transaction_id=(.+)', location).group(1)
        return affiliate_code

    @classmethod
    def get_resource_id(cls):
        ah_resource = cls.provider.get_ah_details_from_config()
        resource_id = ah_resource['resource_id']
        return '{0}'.format(resource_id)

    @classmethod
    def get_resource_name(cls):
        ah_resource = cls.provider.get_ah_details_from_config()
        resource_name = ah_resource['resource_name']
        return '{0}'.format(resource_name)

    @classmethod
    def get_wait_in_seconds(cls):
        wait_in_seconds = cls.provider.get_delay_time_in_seconds()
        time_to_sleep = wait_in_seconds['wait_in_seconds']
