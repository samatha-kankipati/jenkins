...
class AtomHopperProvider(BaseProvider):
    ...
    def __init__(self, url, config, username=None, api_key=None,
        ...
        serialize_format = serialize_format or self.config.misc.serializer
        deserialize_format = deserialize_format or self.config.misc.deserializer
    ...
    def events_by_tenantId_par(self, tenantId, days_back=1, limit=1000):
        ...
        if resp.entity is not None:
            if getattr(resp.entity, 'events') is not None:
                provider_response.entity = self._sort_events(resp.entity.events)
        else:
    ...
    def search_past_events_by_attribute(self, attribute, attribute_regex,
        ...
        return self._search_events_by_attribute(atom_feed_domain_object,
    ...
    def search_compute_events_by_attribute(self, attribute, attribute_regex,
                                           atom_hopper_feed_limit=None,
                                           atom_hopper_pagination_limit=None):
        if atom_hopper_feed_limit is None:
            atom_hopper_feed_limit = (self.config.compute_api
                                      .atom_hopper_feed_limit)
