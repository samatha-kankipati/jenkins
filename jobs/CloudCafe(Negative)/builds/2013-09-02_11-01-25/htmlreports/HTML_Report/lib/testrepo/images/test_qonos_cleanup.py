...
class TestQonosCleanup(BaseImagesFixture):
    ...
    def _cleanup_schedules(self):
        ...
        for schedule in schedules_to_delete:
            try:
                resp = self.images_provider.schedules_client. \
                    delete_schedule(schedule.id)
                if not resp.ok:
                    self.images_provider.logger.error(resp.content)
            except ItemNotFound:
    ...
    def _cleanup_jobs(self):
        ...
        for job in jobs:
            if (job.status.lower() == status_done
                or job.status.lower() == status_error
                or job.status.lower() == status_cancelled):
                    jobs_to_delete.append(job)

        for job in jobs_to_delete:
            try:
                resp = self.images_provider.jobs_client.delete_job(job.id)
                if not resp.ok:
                    self.images_provider.logger.error(resp.content)
            except ItemNotFound:
    ...
    def _cleanup_workers(self):
        ...
        for worker in workers_to_delete:
            try:
                resp = self.images_provider.workers_client. \
    ...
    def _cleanup_instances(self):
        ...
        for instance in instances:
            ...
            if get_server.tenant_id == tenant_id:
                resp = self.images_provider.servers_client. \
    ...
    def _cleanup_snapshots(self):
        ...
        for snapshot in snapshots:
            if (hasattr(snapshot.metadata, 'image_type')
                and (hasattr(snapshot.metadata, 'owner_id'))
                and (snapshot.metadata.image_type == 'snapshot')
                and (snapshot.metadata.owner_id == tenant_id)):
                    resp = self.images_provider.nova_images_client. \
