...
class IdentityConfigQuickCheck(BaseTestFixture):
    ...
    def setUp(self):
        ...
        assert prov_resp.entity is not None, 'Error deserializing identity authentication response, response entity object is none'
    ...
    def test_identity_client(self):
        ...
        assert self.access.user.get_role(name='identity:user-admin') is not None
        assert self.access.user.get_role(id='3', name='identity:user-admin') is not None
