...
class HardRebootServerWithVolumeAttached(BlockStorageLinuxIntegrationFixture):
    """Verifies correct volume behavior for server reboot scenarios where a
    volume is attached
    """

    @attr('smoke')
    def test_hard_server_reboot_with_volume_attached(self):
        ...
        assert resp.ok, \
            ("Volume did not return to the 'in-use' status after"
             " the server it was attached to was hard-rebooted")

    @attr('smoke')
    def test_volume_write_after_hard_server_reboot(self):
        pass
