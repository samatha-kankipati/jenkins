...
from ccengine.providers.identity.v2_0.identity_api import IdentityClientTypes
...
class AdminTokenTest(IdentityAdminFixture):
    ...
    def test_revoke_token(self):
        ...
        self.assertEquals(auth_resp.status_code, 200, 'Exepected response '
                          'code {0} actual {1}'.format(200,
                          auth_resp.status_code))
        ...
        self.assertEquals(list_users_resp.status_code, 200, 'Exepected '
                          'response code {0} actual {1}'.format(200,
                          list_users_resp.status_code))
        #check check token
        check_resp = self.admin_client.check_token(auth_resp.entity.token.id)
        self.assertEquals(check_resp.status_code, 200, 'Exepected response '
                          'code {0} actual {1}'.format(200,
                          check_resp.status_code))
        #revoke token
        revoke_resp = self.admin_client.revoke_token(auth_resp.entity.token.id)
        self.assertEquals(revoke_resp.status_code, 204, 'Exepected response '
                          'code {0} actual {1}'.format(204,
                          revoke_resp.status_code))
        #should not be allowed
        list_users_resp = token_client.list_users()
        self.assertEquals(list_users_resp.status_code, 401, 'Exepected '
                          'response code {0} actual {1}'.format(401,
                          list_users_resp.status_code))
        check_resp = self.admin_client.check_token(auth_resp.entity.token.id)
        self.assertEquals(check_resp.status_code, 404, 'Exepected response '
                          'code {0} actual {1}'.format(404,
                          check_resp.status_code))
        validate_resp = self.admin_client.validate_token(
            auth_resp.entity.token.id)
        self.assertEquals(validate_resp.status_code, 404, 'Exepected response '
                          'code {0} actual {1}'.format(404,
    ...
    def test_authentication_response(self):
        ...
        create_user = self.admin_client.add_user(username=username,
                email=email, enabled=True, defaultRegion=defaultRegion,
                domainId=domainId, password=password)

        update_cred = self.admin_client.update_user_credentials(
                create_user.entity.id, username, apiKey)
        ...
        self.assertEquals(auth_resp_password.entity.token.authenticatedBy[0],
                'PASSWORD', 'Expected PASSWORD, received %s'
                % auth_resp_password.entity.token.authenticatedBy)
        self.assertEquals(auth_resp_apikey.entity.token.authenticatedBy[0],
        ...
        self.assertEqual(hard_delete_user.status_code, 204,
