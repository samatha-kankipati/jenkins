from testrepo.common.testfixtures.dnsaas import Dnsaas_ServerIntegrationFixture
import time
import json


class SmokeTest(Dnsaas_ServerIntegrationFixture):
    ...
    def test_ptr_crud(self):
        data = []
        name = self.name
        rel_id = self.testserver.id
        data.append(self.testserver.addresses.public.ipv4)
        data.append(self.testserver.addresses.public.ipv6)

        type = self.type
        href = '{0}/{1}' .format(self.href, rel_id)
        rel = self.rel
        api_response = self.ptr_provider.client.delete_ptr_lb(rel_id)
        callback = self.ptr_provider.test_wait_for_status(api_response)
