...
class TestQonosEnableScheduledImagesPositive(BaseImagesFixture):
    ...
    def test_enable_sch_images_for_already_enabled_w_invalid_ret(self):
        ...
        list_images_obj = self.images_provider.nova_images_client.list_images()
        self.assertEquals(list_images_obj.status_code, 200,
                          msg.format('status_code', 200,
                                     list_images_obj.status_code))

        for image in list_images_obj.entity:
            image_names.append(image.name)

        self.assertTrue(snapshot_name in image_names,
